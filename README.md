...
- **コード補完**: INSERTモードで1文字入力するたびに `CompletionIndex`（JDKクラス名・プロジェクト内シンボル）から候補を自動検索しポップアップ表示。`Ctrl+Space` で手動トリガー、`↑`/`↓` で候補選択、`Tab`/`Enter` で確定挿入、`Escape` で閉じる。カーソル移動・改行・単語削除などで自動的に閉じる
- **作業ディレクトリ管理**: `WorkingDirectoryManager` が起動時引数・前回値・ホーム・JVM起動ディレクトリの優先順で作業ディレクトリを自動決定。`:pwd` で確認、`:cd <path>` で変更、メニューの `File > Set Working Directory...` でダイアログから変更可能。`:grep`・`:rename`・ファイル名検索・telescope はすべてこの作業ディレクトリを基準に動作
- **チュートリアル**: `:tutor`（または `:tutorial`）コマンドで vimtutor 形式の対話型チュートリアルを開く。説明を読みながら実際にそのバッファを編集して、移動・削除・挿入・ヤンク/ペースト・VISUALモード・検索・コマンドモードから、このエディタ独自のEmacs式移動・Space リーダーキー・Java編集支援・プロジェクト機能まで一通り体験できる。`:q` で終了、`Ctrl+U` で元のバッファに戻れる
- **エントリーポイント参照**: `:main java` / `:main javac` コマンドで、`java`/`javac` コマンドを実行したときに実際に呼ばれる起動点（launcher entry point）へジャンプ。下記「`:main` コマンド」参照
...
### `:main` コマンド（java/javac の実際の起動点へジャンプ）

`java`/`javac` コマンドを実行したときに実際に呼ばれる「起動点（launcher entry point）」へジャンプするコマンド。HotSpot本体（`JVM_GC` 等の実行時関数）ではなく、あくまで launcher が最初に main class を解決・起動する箇所を指す。

| コマンド | ジャンプ先 |
|----------|-----------|
| `:main java` | `java` launcher の native エントリポイント（`src/java.base/share/native/launcher/main.c` の `main()`。ここから `JLI_Launch()` が呼ばれ、コマンドライン引数から実行する main class が解決される）。`java Foo.java` のような単一ソース実行も同じ native `main()` を通り、その後 `LauncherHelper` 内部でソースファイルかクラスファイルかが判定される |
| `:main javac` | `com.sun.tools.javac.Main.main(String[] args)`（jdk.compiler モジュール）。javac には専用の native/JNI 実装は無く、launcher からこの Java メソッドが直接呼ばれる |

対応表は `dev.javatexteditor.analysis.EntryPointIndex` に集約しており、`jar`/`javadoc`/`jshell` 等への対応はここにエントリを追加するだけで拡張できる。

- ターゲット名は大文字小文字を区別しない（`:main JAVA` も可）。
- 引数なし（`:main`）や未対応ターゲット（例: `:main foo`）を指定するとステータスバーにエラーメッセージ（対応ターゲット一覧つき）を表示するだけで、バッファには影響しない。
- ジャンプ先が native ソース（`java`）の場合は `lib/openjdk-native/`（`scripts/setup.sh` で取得）、Java ソース（`javac`）の場合は `lib/src.zip` が必要。未取得の場合は "not available" エラーを表示する（他の jdk-source 系コマンドと同じ graceful degradation）。
- 開いた疑似バッファは他の jdk-source 系コマンド（`K`・`gr`）と同じ挙動: `q` で元のバッファに戻る。
...
| `:cd <path>` | 作業ディレクトリを変更（相対パスは現在の作業ディレクトリから解決）。`:grep`・`:rename`・ファイル名検索・telescope の基準ディレクトリも追従して切り替わる |
| `:tutor` / `:tutorial` | このエディタの操作を学べる対話型チュートリアルを開く（vimtutor 形式。下記参照） |
| `:main <target>` | `java`/`javac` コマンドの実際の起動点（launcher entry point）へジャンプ。下記「:main コマンド」参照 |
...
| `Ctrl+Space` | コード補完ポップアップを起動（候補は自動でも1文字入力ごとに更新される） |
...
│   ├── tutorial/
│   │   └── TutorialTest.java         # Tutorial.CONTENT検証 + :tutor コマンド統合テスト
│   └── ui/
...
`:grep`・`:rename`・ファイル名検索・telescope ピッカーなどの「親ディレクトリ／基準ディレクトリ」は、すべて `WorkingDirectoryManager` が管理する単一の作業ディレクトリを参照します。

**初期値の決定順（上が優先・最初に有効なディレクトリが採用される）**:

1. コンストラクタに渡したヒント（起動時に開いたファイルの親ディレクトリ）
2. `java.util.prefs.Preferences` に永続化された前回の作業ディレクトリ
3. ユーザーのホームディレクトリ（`user.home`）
4. JVM 起動ディレクトリ（`user.dir`）

「有効」の判定は `Files.isDirectory(path)` のみ（存在し、かつディレクトリであること）。`null` や無効なパスは次の候補へフォールバックします。

**変更時の検証（`setWorkingDirectory()`）**:

- `null` → エラー
- 絶対パス化・正規化（`toAbsolutePath().normalize()`）後、存在しない → エラー
- ディレクトリでない → エラー
- 読み取り権限がない（`Files.isReadable()` が false）→ エラー
- すべて通過した場合のみ採用し、`Preferences` へ永続化、登録済みリスナー（全エディタの `projectRoot` 更新・`EditorCanvas` のステータスバー表示・`JFrame` タイトル更新）へ通知

**変更方法**:

- `:cd <path>` コマンド（相対パスは現在の作業ディレクトリから解決）
- メニュー `File > Set Working Directory...` のディレクトリ選択ダイアログ

`ModalEditor.getProjectRoot()` は `projectRoot`（`Main` から `setProjectRoot()` で注入される `WorkingDirectoryManager` の値）が未設定の場合のみ `user.dir` にフォールバックします。

### プロジェクト全文検索: ProjectSearcher

`:grep <pattern>` コマンドで、現在の作業ディレクトリ（`WorkingDirectoryManager` 管理。詳細は前節）配下のファイルを正規表現で検索します。
...
=== dev.javatexteditor.tutorial.TutorialTest ===                   PASS: 9 / 9
...
合計: 1018 テストケース全 PASS（上記は抜粋。telescope/filer/コード補完等のテストクラスを含む全件は scripts/test.sh 実行結果を参照）
```
