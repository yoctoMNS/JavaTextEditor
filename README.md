...
- **モーダル編集**: Vim式のNORMAL/INSERT/COMMANDモードを採用
...
ModalEditor.processKey(keyCode, keyChar, modifiers)
    ├── NORMAL モード: h/j/k/l 移動、i/a/o でINSERT移行、u/Ctrl+R でアンドゥ/リドゥ、: でCOMMAND移行
    ...
    EditorCanvas.setText() / setCursor()
        / setInsertMode() / setCommandLineText()   ← 再描画
```
...
- 画面最下部にステータス行（`-- NORMAL --` / `-- INSERT --` / `:コマンド入力中` / 操作結果メッセージ）
...
=== dev.vimacs.editor.ModalEditorTest ===         PASS: 65 / 65
=== dev.vimacs.ui.EditorCanvasTest ===            PASS:  8 /  8
```
