...
- **モーダル編集**: Vim式のNORMAL/INSERT/COMMAND/VISUALモードを採用
...
| `v` | VISUALモードへ（文字単位選択） |
| `x` | カーソル位置の1文字を削除 |
| `p` | ヤンクレジスタの内容をカーソルの後ろに貼り付け |
| `P` | ヤンクレジスタの内容をカーソルの前に貼り付け |
...
| `y` | 選択範囲をヤンクしてNORMALモードへ戻る |
| `d` | 選択範囲を削除してヤンクし、NORMALモードへ戻る |
...
- VISUALモード: 選択範囲をアクセントカラーでハイライト表示
- 画面最下部にステータス行（`-- NORMAL --` / `-- INSERT --` / `-- VISUAL --` / `:コマンド入力中` / 操作結果メッセージ）
...
=== dev.vimacs.editor.ModalEditorTest ===         PASS: 91 / 91
=== dev.vimacs.ui.EditorCanvasTest ===            PASS: 10 / 10
```
