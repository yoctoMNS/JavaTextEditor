...
- **C言語開発支援**: `gcc`/`clang` 連携によるコンパイル・実行（`F10`/`F11`/`F12`）、`gcc -fsyntax-only` によるエラー・警告のインライン表示、`#include` の自動挿入（`.c`/`.h` を開くと有効。詳細は [docs/manual/11-c-tooling.md](docs/manual/11-c-tooling.md)）
