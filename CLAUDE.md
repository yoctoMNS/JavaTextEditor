...
### バグ修正: Ctrl+Shift+T / :term で画面が更新されず「起動されない」ように見える不具合

- **不具合報告**: 「Ctrl+Shift+T / :term で対話型ターミナルモードが起動されません」。調査したところ、内部状態（`mode`・`terminalBuffer`・実プロセス）はいずれも正常に起動しているにもかかわらず、**画面に一切反映されない**ケースがあることが判明した（実プロセスが起動しない訳ではなく、"起動したように見えない"バグ）。
- **原因1（ステータス行）**: `EditorCanvas.drawStatusLine()`のモードラベル判定（`insertMode`/`visualMode`系のみ）に`Mode.TERMINAL`用の分岐が存在せず、常に`-- NORMAL --`と表示されていた。`EditorCanvas`に`terminalMode`フィールド・`setTerminalMode()`/`isTerminalMode()`を追加し、ラベル判定に`-- TERMINAL --`分岐を追加したが、**`ModalEditor.syncCanvas()`側で`canvas.setTerminalMode(mode == Mode.TERMINAL)`を呼ぶこと自体を実装当初は失念しており**、この配線漏れ単体でもステータス行が誤表示され続けるバグになっていた（`setInsertMode(mode == Mode.INSERT)`のすぐ下に追加して解消）。
- **原因2（画面全体が更新されない、より重大な方）**: `toggleTerminalMode()`（Ctrl+Shift+Tのグローバルキーディスパッチャから`processKey()`を経由せず直接呼ばれる公開メソッド）が、状態変更後に`syncCanvas()`を呼んでいなかった。`processKey()`経由の操作は末尾で自動的に`syncCanvas()`が呼ばれるが、Ctrl+Shift+Tはこの経路をバイパスするため、**シェルプロセスが起動直後に何も出力しない環境（起動メッセージの無いシェル・環境依存でstdoutのflushが遅延するケース等）では、非同期の出力コールバック（`appendTerminalOutput`経由の`syncCanvas()`）も発生せず、ユーザーが次に何かキーを押すまで画面が一切更新されないまま**になっていた。ユーザーから見ると「Ctrl+Shift+Tを押しても何も起きない＝起動していない」に見える、今回の不具合報告の主因と考えられる。本プロジェクトで過去に全く同じ原因で発生した「クラスパス入力プロンプトがキー入力なしでは描画されない不具合」（`enterClasspathInput()`の節参照）と同根であり、`showCompileResult()`/`showRunOutput()`/`enterMainClassPicker()`が採用したのと同じ「`processKey()`を経由しない公開メソッドは自身の末尾で`syncCanvas()`を呼ぶ」というルールを、当時見落としたまま今回の`toggleTerminalMode()`実装に適用し忘れていた。
- **修正**: `toggleTerminalMode()`自身の末尾に`syncCanvas()`を追加した（Main.java側で個別に呼ぶのではなく、`ModalEditor`の公開メソッド自身が責務を持つ設計にし、以後この種のバイパス経路が増えても同じ見落としを防ぐ）。`enterTerminal()`にも、直前の操作で残った`statusMessage`が`syncCanvas()`の`commandLineText`優先順位（`statusMessage` > モードラベル）により「-- TERMINAL --」表示より優先されて表示され続けてしまう問題があったため、`mode = Mode.TERMINAL`と同時に`statusMessage = ""`をクリアするようにした。
- **テスト**: `test/dev/javatexteditor/editor/TerminalModeTest.java`に`testSyncCanvasReflectsTerminalModeAndClearsStaleStatusMessage()`を追加（実`EditorCanvas`インスタンスを使い、`:w`でstatusMessageを汚した状態から`:term`後に`canvas.isTerminalMode()==true`かつ`statusMessage`がクリアされること、`toggleTerminalMode()`でのトグル後に`canvas.isTerminalMode()==false`に戻ることを検証）。加えてXvfb+`java.awt.Robot`で「statusMessageを汚す→Ctrl+Shift+T→50ms後に即座にスクリーンショット（非同期出力を待たない）」というシナリオを手動確認し、ステータス行が即座に`-- TERMINAL --`に切り替わり、シェルの実出力も表示されることを確認した。
