...
- **追記: F10/F11/F12が`src`/`bin`フォルダの深い階層に`:cd`で潜り込んだ状態でも、プロジェクトルートを自動検出するようにした**。
  - **経緯**: `ProjectBuilder.binDirFor()`は元々`src`を持つ祖先ディレクトリを遡って`bin/`の置き場所だけを解決していたが（本ファイル前半の「`bin/`の配置場所を`src`フォルダの親ディレクトリに固定した」節）、実際にソースを走査してコンパイルする`compile(Path projectRoot)`・main クラスを探す`MainClassFinder.findMainClasses(projectRoot)`・実行時の作業ディレクトリ（`ProcessBuilder.directory()`）は、いずれも`ModalEditor.getProjectRoot()`（＝`:cd`で設定した生の作業ディレクトリ）をそのまま受け取っていた。そのため、パッケージの深い階層（例:`src/dev/javatexteditor/`）に`:cd`していると、`bin/`は正しくプロジェクトルート直下に置かれるのに、コンパイル対象・main探索対象はそのパッケージ配下だけに限定されてしまうという不整合があった。ユーザーから「`src`または`bin`が所属しているフォルダを自動でプロジェクトルートに設定してほしい（`:cd`でルートまで登り直す手間・ファイル検索に無関係なファイルが混ざる問題・新規ファイル作成時のディレクトリの深さの問題を解消するため）」という要望を受けて解消した。
  - **`ProjectBuilder.resolveProjectRoot(Path)`（新設・`public`、旧`private resolveProjectBaseDir`を改名・一般化）**が唯一の解決経路。`src`を直下に持つ最初の祖先ディレクトリ、それが無ければ`.class`ファイルを含む`bin`を直下に持つ最初の祖先ディレクトリを返す。どちらの祖先も見つからなければ渡された`projectRoot`自身にフォールバックする。`binDirFor()`はこのメソッドに委譲するだけになった。
  - **`Main.java`に`resolveBuildRoot(ModalEditor)`ヘルパーを新設**し、F10/F11/F12のすべてのエントリポイント（`doCompile()`・`triggerRun()`・`triggerCompileAndRun()`・`setOnRunMainClassSelected`コールバック）が`editor.getProjectRoot()`の生値ではなく`resolveBuildRoot(editor)`（＝`PROJECT_BUILDER.resolveProjectRoot(editor.getProjectRoot())`）を経由するように統一した。これにより`bin/`の配置場所・ソース走査対象・main探索対象・実行時の作業ディレクトリが常に同じ解決結果を参照する（従来は`binDirFor()`内部だけがこっそり祖先を遡り、他は生の`projectRoot`を使うという非対称な状態だった）。
  - **`bin`判定は「ディレクトリの存在」だけでは行わない（重要な安全弁）**: 実装中に、単純な`Files.isDirectory(dir.resolve("bin"))`判定だと、ほぼ全てのUnix系OSに実在する無関係なシステムディレクトリ（`/bin`・`/usr/bin`等）に祖先を遡って行き着いてしまい、`src`が見つからない場合に最悪ファイルシステムルート付近を"プロジェクトルート"と誤認識する（＝ディスク全体を走査・コンパイル対象にしようとする）というバグを実際に作り込みテストで検出した。`resolveProjectRoot()`の`bin`側の判定は、既存の`hasCompiledClasses()`と共通化した`containsClassFile(Path)`ヘルパーで「`bin`ディレクトリが実在し、かつ配下に`.class`ファイルを1つ以上含む」ことまで確認してから初めて祖先候補として採用する。`src`判定にはこのガードは適用していない（`src`という名前がシステムディレクトリと衝突する現実的なリスクは無いため）。
  - **テスト**: `ProjectBuilderTest`に`testResolveProjectRootClimbsToBinAncestorWhenNoSrc()`（`.class`を含む`bin/`を持つ祖先まで正しく遡ること）・`testResolveProjectRootIgnoresEmptyBinAncestor()`（`.class`を含まない空の`bin/`は無視すること＝上記安全弁の回帰テスト）・`testResolveProjectRootPrefersSelfWhenBothSrcAndBinArePresent()`を追加した。
  - **意図的にスコープ外とした点**: `Main.java`側の実際の配線（`resolveBuildRoot()`がGUI経由で正しく呼ばれること）はF10/F11/F12の他の部分と同様GUI依存のため自動テスト対象外（既知のギャップ）。祖先を遡る際の探索範囲に上限（例: ホームディレクトリを超えない等）は設けていない。`src`/`bin`のいずれも実在しない場合は従来どおり`projectRoot`自身にフォールバックし、この点は変更していない。
