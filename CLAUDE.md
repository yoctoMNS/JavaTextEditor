...
「軽量エディタ」の主張と実装の間に4つの深刻なギャップ（①`PieceTable`のピース結合欠如による編集セッション全体のO(K²)劣化・②`syncCanvas()`の1キー入力あたり4回のO(n)全文再構築・③Shift+K/`gr`/`:grep`のO(ファイル数)逐次走査とタイムアウト後のスレッド残留・④編集中の文書サイズ比例メモリチャーン）があることを実コード調査で確認し、解消計画を策定済み。

- **計画書**: `docs/PERF_REFACTORING_PLAN.md` — 問題の根拠（ファイル:行番号）・ベースライン実測・ゴール/非ゴール・完了条件。
- **指示書**: `docs/PERF_REFACTORING_INSTRUCTIONS.md` — 実行手順の正。**この作業に着手するセッションは、必ず指示書だけに従って作業すること**（変更コード全文・テストコード全文・ゲート判定コマンド・コミット/PR/mainマージ手順・STOP条件まで定義済み。指示書の完了ゲートを全て満たした場合のmainマージは2026-07-15のユーザー指示により事前承認されている）。

| Phase | 対象 | 状態 |
|---|---|---|
| 1 | `PieceTable`: 連続挿入のピース結合・`length()`のO(1)キャッシュ・`addBuffer.toString()`コピー排除・`offsetOfLine()`の全文再構築排除 | 未着手 |
