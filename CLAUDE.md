...
- **キー割り当て**: NORMAL/INSERT両モードで `Ctrl+>`（次のエラー）・`Ctrl+<`（前のエラー）。物理的には Shift+ピリオド/Shift+カンマのため `KeymapRegistry` 上は `KeyEvent.VK_PERIOD`/`KeyEvent.VK_COMMA` + `CTRL_DOWN_MASK | SHIFT_DOWN_MASK` で束縛した（アクション名 `compile.error.next`/`compile.error.prev`）。当初ユーザーからはCtrl+N/Ctrl+Mでの依頼だったが、INSERTモードのCtrl+Nは既にEmacs式カーソル下移動（`.claude/skills/keymap-conflict-resolution/SKILL.md`参照）に確定済みで衝突するため、ユーザーに確認しCtrl+>/Ctrl+<へ変更した（Ctrl+Mは未使用だったが、依頼変更に伴いこちらも変更）。
- **既存の `[g`/`[d`（`jumpToNextDiagnostic`/`jumpToPrevDiagnostic`）との違い**: `[g`/`[d` は現在編集中の1ファイル内の診断（`CompileDiagnostic`、`canvas.getDiagnostics()`/`localDiagnostics`）のみを対象にする既存機能。今回の`Ctrl+>`/`Ctrl+<`は**プロジェクト全体**（`getBuildRoot()`配下の全`.java`）を対象にする別機能のため、既存メソッドには一切手を入れず`jumpToProjectCompileError(boolean forward)`を新設して分離した。
- **実装**: F10と同じ`dev.javatexteditor.projectbuild.ProjectBuilder.compile(getBuildRoot())`をその場で呼び、返ってきた`BuildResult.diagnostics()`のうち`isError()==true`のものだけを対象にする（警告は対象外、仕様どおり）。`filePath→lineNumber→column`の辞書式順序でソートし、現在の`currentFilePath`+カーソル位置から見て厳密に後/前にある最初の1件へジャンプする。見つからない場合は先頭/末尾へラップアラウンドする（1件しかない場合は同じ位置へ戻る）。
- **EDTブロッキング対策**: プロジェクト全体のjavacコンパイルはgrep検索より重いため、Shift+K等と同じ`withTimeout()`パターンを流用しつつ、専用の`COMPILE_ERROR_NAV_TIMEOUT_MS`（8000ms、既存の`PROJECT_SYMBOL_SEARCH_TIMEOUT_MS`=1500msより長めに設定）で打ち切るようにした。タイムアウト時は「E: compile check timed out」を表示するのみで、既存の`withTimeout(Supplier)`はこの新しい`withTimeout(Supplier, long)`オーバーロードに委譲する形にリファクタリングした（引数なし版は従来どおり1500ms）。
- **エラーが無い場合**: 依頼どおり「コンパイルエラーはありません」をステータス行に表示するのみで、カーソル・バッファとも変更しない。
- **ファイルをまたぐジャンプ**: 対象がバッファと異なるファイルの場合、`jumpToGrepResult()`等と同じ`readFileContentForBuffer()`+`acquireBufferForOpen()`（共有バッファ機構、本ファイル該当節参照）経由で開く。バイナリ/classファイル判定になった場合はエラー表示のみで開かない（コンパイル診断は常に`.java`ソースを指すため通常到達しない防御的分岐）。
- **テスト**: `test/dev/javatexteditor/editor/ProjectCompileErrorNavTest.java`（新設・6テストメソッド/16アサーション）。エラーなし時のメッセージ・複数ファイルにまたがる前方/後方ジャンプ・単一エラーでの折り返し・INSERTモードからの起動を検証。実際のディスク上に一時ディレクトリ＋`.java`ファイルを用意し、本物の`javax.tools.JavaCompiler`を実行させて検証している（F10系の既存テストと同じ手法）。
