...
## 新規 .java ファイル作成時の package 文・クラス定義の自動挿入

「新しくファイルを作成する際にパッケージ文とクラス定義を自動で挿入してほしい。パッケージ名は src フォルダをルートとし、Maven/Gradle にも対応（src/main/java 等の中間フォルダは無関係）。src が見つからない場合は祖先ディレクトリを遡って探す。bin フォルダがあり src がない場合はパッケージ文を挿入せずクラス名だけ挿入する。デフォルトパッケージにも対応」という要望を受けて実装した。

- **唯一の挿入経路は `ModalEditor.loadFromFile()` の新規ファイル分岐（`:e <存在しないパス>`）**。新規ファイル作成はこの1箇所のみ（`grep`で確認済み。`:enew`＝`newBuffer()`はファイルパスを持たない疑似バッファのためパッケージ名を計算する材料がなく対象外）。
- **`dev.javatexteditor.analysis.JavaFileStubGenerator`（新設・Swing非依存の純粋ロジック）** が `Path` を受け取り `Stub(String text, int cursorRow, int cursorCol)` を返す。`.java` 以外の拡張子、またはファイル名がクラス名として不正（`Character.isJavaIdentifierStart/Part` 不成立・`javax.lang.model.SourceVersion.isKeyword` に一致する予約語）の場合は `null` を返し、`ModalEditor` 側は従来通り空文字列・カーソル(0,0)にフォールバックする（他の拡張子・不正ファイル名は非デグレ）。
- **パッケージ名の解決アルゴリズム（`resolvePackageName`）**: 新規ファイルの**親ディレクトリ**から`Path.getParent()`で祖先を1段ずつ遡り、ディレクトリ名が文字列として厳密に`"src"`と一致する最初の祖先を探す（`ProjectBuilder.resolveProjectBaseDir()`の「`src`を子に持つディレクトリを探す」ロジックとは異なり、こちらは「`src`という名前のディレクトリそのもの」を探す点に注意。目的が違うため意図的に別実装にした・共通化していない）。ファイルシステムの実在チェック（`Files.exists`等）は行わない＝パス文字列の解析のみで完結する。これにより、`src/main/java/com/example/`のようにまだ存在しない中間ディレクトリを含む新規パスを`:e`で指定した場合でも、ディレクトリ作成前の時点で正しくパッケージ名を計算できる。
  - 見つかった`src`ディレクトリから新規ファイルの親ディレクトリまでの相対パスをセグメント分解し、Maven/Gradleの慣例的な中間ディレクトリ名（`main`/`test`/`java`/`kotlin`/`groovy`/`scala`/`resources`/`testFixtures`、`NON_PACKAGE_SEGMENTS`定数）を除外してから`.`で連結する。除外後のセグメントが0件（＝`src`直下、またはmain/java等を除いたら何も残らない）の場合は`null`を返し、呼び出し側は`package`文を挿入しない＝**デフォルトパッケージ**として扱う。
  - `src`祖先が最後まで見つからない場合も同じく`null`を返し、`package`文なしのクラス定義のみを挿入する。**`bin`フォルダの有無は一切参照しない**（`src`が無ければ常にクラス名のみ、という単純な1本のフォールバック経路に統一しており、「`bin`があるかどうか」で分岐する専用コードは意図的に追加していない。要望文中の「binフォルダがありながらsrcフォルダがない場合」は、`bin`を`src`の代替アンカーとして誤って使わないことの確認・具体例として読み、`ProjectBuilder.binDirFor()`のような`bin`基準のプロジェクトルート解決とは混同しないよう念のためテストケースとしても明記した）。
- **カーソル位置**: 生成したクラス本体の空行（`public class Foo {`の次の空行）に置く。`package`文ありなら3行目、なしなら1行目（0始まり）。ひな形のレイアウトが固定（`"public class " + className + " {\n\n}\n"`、パッケージありなら先頭に`"package " + pkg + ";\n\n"`を付加）なため、行番号は分岐で決め打ちしている。
- **意図的にスコープ外とした点**: `:enew`（`newBuffer()`）・FILERでのファイル作成相当の機能は無いため対象外。既存ファイルを開く分岐（`Files.exists(p)`が真の場合）は変更していない。パッケージ名セグメントの大文字小文字補正（例: ディレクトリ名がJavaの命名規約に反する場合の警告等）は行わない。
- **テスト**: `test/dev/javatexteditor/analysis/JavaFileStubGeneratorTest.java`（新設・10テスト、純粋ロジック単体）と`test/dev/javatexteditor/editor/NewFileStubTest.java`（新設・4テスト、`ModalEditor`経由で`:e`コマンドから実際にひな形が挿入されることを検証）。Maven風`src/main/java`・Gradle風`src/test/java`・`src`直下のデフォルトパッケージ・深いネスト・`bin`はあるが`src`が無いケース・`src`も`bin`も無いケース・`.java`以外の拡張子・不正な識別子（ハイフン含む）・予約語ファイル名、を網羅。
