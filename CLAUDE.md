...
### TERMINALモードでのTab単語補完（2026-07-21）

「WindowsやLinuxでもTerminalモードでもTAB入力でワード補完ができるようにしてほしい」という依頼。既存のAlt+/単語補完（本ファイル「単語補完（Alt+/）の設計決定事項」節参照）と同じ`WordIndex`をTERMINALモードでも使えるようにした。

- **OS別分岐は不要だった**: TabキーはOSネイティブのターミナル機能ではなく、`Main.java`のグローバル`KeyboardFocusManager`ディスパッチャ（F10/F11/Ctrl+Shift+T等と同じ仕組み）が`ed.processKey()`へそのまま渡す。この経路はJava標準のAWTイベントディスパッチのみに依存しWindows/Linux/macOSで同一に動作するため（Swingのデフォルトのフォーカス移動用Tab処理より先にこのディスパッチャが消費してしまうため、素のフォーカス移動に食われる心配もない。既存の`insert.tab`アクション＝INSERTモードでのTabインデントが同じ経路で既に正しく動いていることからも確認済み）、`processTerminalKey()`側にOS判定を入れる必要はない。
- **Tab押下時の処理は既存のAlt+/トリガー（`triggerWordCompletion()`）をそのまま呼ぶだけ**にした。`buffer`はTERMINALモード中`terminalBuffer`（共有バッファ）を指しており、カーソルは`moveCursorToTerminalEnd()`で常にバッファ末尾（＝プロンプト直後の入力中の行）に追従しているため、`extractCompletionPrefix()`等の既存の「カーソル直前の識別子」ロジックはTERMINALモードでも変更なしでそのまま正しく機能する。
- **確定（Tab/Enter）・ナビゲーション（↑↓）・キャンセル（Esc）のキー割り当てはINSERTモードの補完ポップアップと完全に統一**した。ポップアップが開いている間はこれらのキーを`processTerminalKey()`冒頭で最優先処理し、それ以外のキーが来た場合は`dismissCompletion()`してから通常のターミナル処理（ローカルエコー・Backspace等）へフォールスルーする。**Escは「補完ポップアップを閉じるだけ」を優先し、ポップアップが開いていない場合にのみ既存の「TERMINALモードごと抜ける」動作（本ファイル前節参照）に委ねる**よう判定順序をswitch内に組み込んだ（INSERTモードで補完ポップアップのEscが`enter.normal`より優先されるのと同じ設計）。
- **`applyCompletion()`（INSERTモード用の既存メソッド）は`buffer`のみを書き換え、Enter押下時にシェルへ送信される`terminalPendingInput`（ローカルエコーと並行して保持している「送信待ちの1行」用の別のバッファ）の存在を知らない**。そのままでは補完確定後に送信される行が「補完前の切り詰められたプレフィックス」のままになるバグを作り込んでしまうため、`applyTerminalCompletion()`（新設）を用意した。確定前に選択中候補のラベルと`completionPrefix`の長さを退避し、`applyCompletion()`（buffer編集・カーソル更新・ポップアップ終了）を呼んだ後、`terminalPendingInput`から同じ長さのプレフィックスを削って補完後のラベルを追記する、という2段構えにしている。
- **意図的にスコープ外とした点**: 文字入力のたびに自動でポップアップを開く「as-you-type」動作（INSERTモードの`recheckCompletion()`が1文字挿入ごとに呼ばれる仕組み）はTERMINALモードには持ち込まなかった。依頼が「TAB入力で」の補完を求めていたことと、シェルの通常の入力体験（1文字打つたびにポップアップが出るのは煩わしい）を壊さないための判断。ポップアップが開いた状態で通常文字を打つと（候補選択の続行ではなく）ポップアップを閉じてその文字を素直にローカルエコーする、という単純な仕様にした。
- **テスト**: `test/dev/javatexteditor/editor/TerminalModeTest.java`に4テスト追加（計69テスト）。Tabでポップアップが開き候補にヒットすること、確定後に`terminalPendingInput`が正しく更新されEnter送信時に補完後の完全な単語がwrite callbackへ渡ること、Escはポップアップだけを閉じTERMINALモード自体は維持されること、通常文字入力でポップアップが閉じ通常のローカルエコーへ戻ることを検証。実際のOS別Tabキーイベント配送（Windows実機でのIME/フォーカス挙動等）は他のGUI依存機能と同じ既知のテストギャップとして残る。
