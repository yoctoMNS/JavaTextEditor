...
## バグ修正: 右から左/下から上へのドラッグで矩形選択のハイライトが崩れる不具合

- **症状**: `Ctrl+V`の矩形選択で、左から右（列が増える方向）へのドラッグは正しく描画されるが、
  右から左（列が減る方向）へのドラッグを含む選択（同一行内・複数行にまたがる場合の両方）で
  ハイライトが正しく描画されなかった。
- **原因**: `EditorCanvas.drawSelectionHighlight()`は`selAnchorRow/Col`と`selCursorRow/Col`から
  描画用の`r1/c1`(左上)〜`r2/c2`(右下)を求める際、**文字単位選択(VISUAL)と矩形選択(VISUAL BLOCK)で
  同じswapロジック**（`if (r1 > r2 || (r1 == r2 && c1 > c2)) { スワップ }`）を使っていた。
  このロジックは「行→列の順で並べた1本の連続テキストの前後関係」を判定するものであり、
  行が変わらない（同一行内）場合のみ列のmin/maxとして正しく機能する。行を跨ぐドラッグ
  （例: anchor=(0,8) → cursor=(1,2)、右上から左下へ）では`r1(0) < r2(1)`のため条件が
  偽になりswapが発生せず、`c1=8, c2=2`という`c1 > c2`の壊れた状態のまま描画関数に渡っていた。
  本来のモデル層（`ModalEditor`側の`r1=Math.min(anchorRow,cursorRow)`等、本SKILL冒頭の
  「列の扱い」節参照）は最初から行・列を独立にmin/maxしているため、描画側だけがこの前提から
  ずれていた。
- **修正**: `EditorCanvas`に`static int[] normalizeSelectionBounds(boolean blockMode, int anchorRow,
  int anchorCol, int cursorRow, int cursorCol)`を新設した。`blockMode==true`の場合は行・列を
  それぞれ独立に`Math.min`/`Math.max`する（`ModalEditor`側の矩形選択ロジックと同じ正規化）。
  `blockMode==false`（VISUAL/VISUAL_LINE）は従来通り行優先のswapロジックのままにした
  （文字単位選択は「1本の連続テキスト」としての前後関係が本質であり、列だけ独立にmin/maxすると
  逆に文字の並び順と矛盾するため、こちらは変更していない）。`drawSelectionHighlight()`は
  この共通ヘルパーを呼ぶだけに変更し、`drawSelectionHighlightWrapped()`（折返し時の描画）は
  呼び出し元から正規化済みの`r1/c1/r2/c2`を受け取る既存の設計のため無修正で自動的に直った。
- **テスト**: `test/dev/javatexteditor/ui/EditorCanvasTest.java`に3テスト追加（Test 45〜47、
  計47テスト）。`normalizeSelectionBounds`の単体テスト（block=true/falseそれぞれで4方向の
  ドラッグを検証）と、右上から左下へのドラッグでピクセル上も正しい列範囲がハイライトされることの
  検証。

## VISUAL / VISUAL LINE の `p`（選択範囲を上書き貼り付け）

「VISUALモードで選択している間に`p`を押すと、選択範囲をヤンクレジスタの内容で上書きして貼り付けられるようにしてほしい」
という要望に基づき追加した。従来 VISUAL/VISUAL_LINE には `p`/`P` のバインドが一切存在せず、選択中に押しても
何も起きなかった（`KeymapRegistry.resolve()`が`null`を返すだけ）。

- **`KeymapRegistry`に`Mode.VISUAL`/`Mode.VISUAL_LINE`双方へ`ofChar('p', "paste.over")`を追加**した。
  NORMALモードの`p`(`paste.after`)/`P`(`paste.before`)とは別アクション名にした（選択範囲の上書きという
  異なる意味論のため）。`P`（大文字）はVISUALでは追加していない（Vim本家もVisualの`p`/`P`はどちらも
  同じ「選択範囲を上書き」の意味になり区別がないため、片方だけ実装すれば足りる）。
- **VISUAL（文字単位）**: `deleteSelected()`で選択範囲を削除した後、その削除開始位置に`yankRegister`の
  内容をそのまま`buffer.insert()`する（`yankType`による分岐はしない＝CHAR/LINE/BLOCKいずれの登録内容でも
  生テキストとしてその位置に挿入するだけの単純な設計）。カーソルは挿入した内容の末尾に移動する
  （`pasteCharAfter()`と同じ「挿入末尾に留まる」規約）。
- **VISUAL LINE（行単位）**: `deleteLineRange(r1, r2)`で選択行を削除すると`cursorRow`が`r1`・`cursorCol`が
  `0`になる（既存の`deleteLineRange()`の副作用）。`yankType == LINE`の場合はこの位置で既存の`pasteLineBefore()`
  をそのまま呼べば「削除した行の位置に貼り付け行が来る」という自然な結果になる。`yankType`が`LINE`以外
  （文字単位ヤンク等をVISUAL LINEで上書きに使った場合）は`buffer.insert(offsetOfCursor(), yankRegister)`で
  生テキストとしてその行頭に挿入する（新しい行として整形し直す処理はしない。これも上記VISUALと同じ
  「単純に生テキストとして置く」という設計判断）。
- **意図的に単純化した点**: 本家Vimは選択範囲を上書きした際、削除された元のテキストが無名レジスタに
  入れ替わる（＝ヤンクレジスタの中身と削除された内容が交換される）が、本プロジェクトはこの交換を
  実装していない。`yankRegister`は`paste.over`実行後も貼り付け前の内容のまま変化しない（同じ内容を
  続けて複数箇所に上書きペーストできる、という単純化されたトレードオフ）。既存の`y`/`d`が単一の
  `yankRegister`フィールドのみで動作する設計（レジスタ指定機構が無い）とも整合する。
- **VISUAL BLOCK（矩形選択）の`p`は対象外**とした。矩形選択には既に`I`/`A`/`c`/`r`という専用の
  矩形編集キーが用意されており、「選択範囲を上書き」という意味論を矩形の列範囲にどう適用するかは
  別途仕様判断が必要なため、今回の要望（VISUALモードでのpによる上書き）のスコープ外とした。
- **テスト**: `test/dev/javatexteditor/editor/ModalEditorTest.java`に`testVisualPasteOverChar()`/
  `testVisualLinePasteOverLine()`を追加（計115テスト）。文字単位・行単位それぞれでヤンク→別範囲を
  選択→`p`で上書き→NORMALへ戻ることを検証。
