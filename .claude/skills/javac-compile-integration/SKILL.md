...
| クラス | 場所 | 役割 |
|---|---|---|
| `DiagnosticKind` | `src/dev/vimacs/analysis/DiagnosticKind.java` | ERROR / WARNING の enum |
| `CompileDiagnostic` | `src/dev/vimacs/analysis/CompileDiagnostic.java` | 診断1件を表すレコード（lineNumber, column, message, kind） |
| `CompileAnalyzer` | `src/dev/vimacs/analysis/CompileAnalyzer.java` | JavacTask.analyze() まで実行して診断を収集 |
...
`test/dev/vimacs/analysis/CompileAnalyzerTest.java` に 15 テスト。
