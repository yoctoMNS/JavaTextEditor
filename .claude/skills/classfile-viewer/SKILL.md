...
- **`readFileContentForBuffer(Path)`の`FileLoadResult`にフィールドを追加**した
  （`record FileLoadResult(String text, boolean binary, byte[] rawBytes, byte[] classFileBytes, String classFileDisplayName)`）。
  ...
  生バイト列を`classFileBytes`に格納して返す。**この場合`binary()`は`false`のままにする**
  （`.class`構造ビューは読み取り専用プレビューであり、`:bコマンド`由来の編集可能なMode.BINARYには
  一切入らない。`classFileBytes() != null`という専用の判定だけを別途持たせることで、
  `.class`の3値目の分岐を「バイナリかどうか」の2値判定と混同しないようにしている）。
  **パースに失敗した場合（壊れた`.class`）は例外を握りつぶして既存の`BinaryFileDetector`による
  Mode.BINARYフォールバック（`binary()=true`、`rawBytes()`経由で`enterBinaryMode()`）へ
  素通りする**（この一本の関数が全てのファイルを開く経路の共通入口のため、ここでの判定だけで済む）。
  **`.class`の読み取り専用プレビューとMode.BINARY（編集可能なhexdumpエディタ、`:b`コマンド由来。
  binary-file-open/`:b`コマンド周りのCLAUDE.md該当節参照）は別物である点に注意**: 前者は
  `currentFilePath`を`null`のままにして保存不可能、後者は`currentFilePath`を実パスのまま維持し
  `:w`で保存できる。壊れた`.class`がMode.BINARYへフォールバックした場合は「編集可能な生バイナリ」
  として扱われる（構造解析には失敗したが、せめてバイト単位では編集・保存できるようにするため）。
  この関数を呼ぶ全5箇所の一覧は上記CLAUDE.md該当節を参照。
