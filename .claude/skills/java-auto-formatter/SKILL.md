...
## 既知の不具合と修正（2026-08-19）

- **フィールド初期化子中の呼び出し式をメソッド宣言と誤認識する不具合**: `private Font font = new Font(Font.MONOSPACED, Font.BOLD, 30);` のようなフィールドが、コンストラクタの直後（本来のフィールド群の位置ではない）に取り残される不具合があった。原因は `classify()` の `NAME_BEFORE_PAREN.matcher(rest).find()` が `rest` 文字列全体を対象に検索するため、初期化子中の `new Font(` にマッチし「メソッド `Font`」として誤分類されていたこと（そのままコールグラフソートに巻き込まれ、`Font`はアルファベット順で他のメソッドより手前に来るためコンストラクタ直後に出力されていた）。
  - 修正: `indexOfTopLevelAssign()` を追加し、`rest` 内でトップレベル（丸括弧・角括弧の深さ0）に現れる代入 `=`（`==`/`!=`/`<=`/`>=` は除外）の位置を求め、`NAME_BEFORE_PAREN` のマッチ開始位置がその `=` より後ろであればメソッドとみなさず `FIELD` にフォールバックするようにした。
  - 回帰テスト: `JavaMemberFormatterTest` Test 27（`new Font(...)` を持つフィールド初期化子がフィールド群に留まることを検証）。
