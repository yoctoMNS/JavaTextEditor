...
3. **軽量コールグラフ**: 各メソッド名について、他の全メソッド名が単語境界（`\bNAME\b`）でボディテキストに出現するかを正規表現でチェックし、隣接リスト（`Map<String, Set<String>>`）を構築する。型解決は一切行わない（同名の無関係なローカル変数・別クラスのメソッド呼び出しと誤認識するリスクはあるが、意図的な軽量トレードオフ）。
...
- **型解決を伴う正確なコールグラフ**: `MethodCallGraphSorter` は単語境界の文字列マッチのみで「呼んでいるらしい」を判定する。同名の別クラスのメソッド・ローカル変数・文字列中のたまたま一致する語を誤って呼び出しとして検出する可能性がある（型解決が無いため原理的に不可避）。速度・メモリ優先のトレードオフとして許容する。
...
```
test/dev/javatexteditor/format/JavaMemberFormatterTest.java   24/24 PASS
