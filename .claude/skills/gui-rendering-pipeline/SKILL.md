...
### 色の定義（`src/dev/vimacs/ui/Theme.java`）
...
### 描画コンポーネント本体（`src/dev/vimacs/ui/EditorCanvas.java`）
...
実装済みテスト: `test/dev/vimacs/ui/EditorCanvasTest.java`（5ケース・全PASS確認済み）

目視確認用PNG生成: `test/dev/vimacs/ui/VisualPreview.java`（`build/preview_*.png`に出力）

## 次に学ぶべきこと

1. ②（`modal-editing-engine`）が完成したら、`setCursor`/`setInsertMode`を実際のキー入力に接続する
