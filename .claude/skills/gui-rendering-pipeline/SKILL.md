...
## `charCellWidth()`に囲み英数字（丸数字①②③等、U+2460-24FF）の判定漏れがあった不具合の修正

- **症状**: 「丸数字（①②③等）がきちんと描画されない」という報告。上記のかぎ括弧・幾何学記号の不具合と同種で、丸数字を含む行で後続文字がグリフの右半分に重なって描画される。
- **原因**: `charCellWidth()`は上記2件の修正時点でも「囲み文字（丸数字①②③等、U+2460-24FF）」を明示的にスコープ外としたままだった（各節の「意図的にスコープ外とした点」参照）。このブロック（Enclosed Alphanumerics）はUnicode East Asian Width上は「Ambiguous」だが、日本語フォント・日本語の文章中では全角幅のグリフとして描画されるため、既存の全角記号・幾何学記号と同じ理由で全角(2)判定が必要だった。
- **修正**: `EditorCanvas.charCellWidth(int codePoint)`に`0x2460-0x24FF`（Enclosed Alphanumerics ブロック全体）を全角(2)判定として追加した。この1箇所の修正だけで`drawLineWithFullWidthSupport()`/`xForCol()`/`drawCursor()`等の描画パイプライン全体に反映される。
- **意図的にスコープ外とした点**: 絵文字（サロゲートペアの補助面）・CJK互換用字（U+F900-FAFF）は今回の不具合報告（丸数字）の再現範囲を超えるため引き続き対象外とした。
- **テスト**: `test/dev/javatexteditor/ui/EditorCanvasTest.java`にTest 5dを新設（計51テスト）。`charCellWidth(0x2460)`（①、ブロック先頭）・`charCellWidth(0x2461)`（②）・`charCellWidth(0x2462)`（③）・`charCellWidth(0x24FF)`（ブロック末尾）がいずれも2であることを検証。

## Java/C構文ハイライトの新規実装・デフォルトテーマをDARK_MODEに変更（2026-07-26）

「参考画像（C/C++コードのダークモード画面）を元に構文ハイライトを設定し直してほしい」という依頼を受け調査したところ、**このエディタには構文ハイライト機能自体がそもそも一切存在しなかった**（全文字が`theme.foreground`固定の単色描画。CLAUDE.mdのロードマップにも該当Skillがない）ことが判明した。「設定し直す」ではなく新規実装であることをユーザーに確認した上で着手した。

- **対象言語・配色はユーザーに`AskUserQuestion`で確認済み**: Java+C/C++両対応、配色は「背景=黒、コメント=赤系、文字列=黄緑、キーワード=太字白、型名=シアン系、プリプロセッサ/マクロ=マゼンタ、数値=紫」（参考画像から抽出した配色をそのまま採用）。
- **新規クラス（`dev.javatexteditor.ui`パッケージ、いずれもSwing非依存の純粋ロジック）**:
  - `SyntaxKind`（enum）: `DEFAULT`/`KEYWORD`/`TYPE`/`STRING`/`COMMENT`/`NUMBER`/`PREPROCESSOR`。`Theme`側の`syntaxXxx`フィールドと1対1対応。
  - `SourceLanguage`（enum）: `NONE`/`JAVA`/`C`。`detect(String filePath)`が拡張子で判定する（`.java`→JAVA、`.c`/`.h`/`.cc`/`.cpp`/`.cxx`/`.hpp`/`.hh`/`.hxx`→C）。㉓`symbol-definition-navigation`の`CDefinitionResolver.isCFilePath()`と同じ拡張子リストを踏襲した別定義（用途が異なるため共通化せず、CLAUDE.mdの「3行の重複は早すぎる抽象化よりよい」方針に従った）。
  - `SyntaxToken`（record）: `(int start, int end, SyntaxKind kind)`。行内の`[start,end)`区間がkindに分類されることを表す。
  - `SyntaxHighlighter`（static専用クラス）: `tokenizeLine(String line, SourceLanguage lang, boolean startInBlockComment)`が1行を逐次スキャンしてトークン列を返す。正規表現ではなく単純な文字走査（`gr`/`:grep`等の既存の軽量ヒューリスティックと同じ思想）で、`//`行コメント・`/* */`ブロックコメント（複数行対応）・`"..."`/`'...'`（バックスラッシュエスケープ対応）・数値（16進数`0x`・アンダースコア区切り・小数・指数・`f`/`d`/`l`/`u`サフィックス）・識別子（キーワード集合・型集合・PascalCase/ALL_CAPS識別子はTYPE扱い）・Cのプリプロセッサ行（行頭が`#`）を分類する。型解決を伴う厳密な分類はしない（既存の`CDefinitionResolver`等と同じ既知の限界）。
- **ブロックコメントの複数行対応とパフォーマンス設計**: `/* */`は複数行にまたがるため、`computeBlockCommentStarts(String[] lines, SourceLanguage lang)`が文書全体を**1回だけ**逐次走査し「各行がブロックコメントの内側から始まるか」を`boolean[]`で事前計算する。`EditorCanvas.setText()`が、渡された`lines`配列の**参照**または`language`が前回と変わった時だけこの事前計算を再実行する（`ModalEditor.canvasTextOwner`と同じ「参照一致による自動失効」パターン。`syncCanvas()`は毎キー入力で`setText()`を呼ぶが、テキスト未変更ならModalEditor側で同じ配列参照が渡されるため実際の再計算は起きない）。可視行だけを毎フレーム`tokenizeLine()`で都度トークン化するため、数十万行規模のファイルでも描画コストは可視行数に比例するのみで、Phase 1〜3の軽量化リファクタリング方針と矛盾しない。
- **`ModalEditor.syncCanvas()`**が`canvas.setLanguage(SourceLanguage.detect(currentFilePath))`を`canvas.setText(...)`より**前**に呼ぶよう配線した（`setText()`内のブロックコメント状態キャッシュが言語変化を検知できるようにするため、呼び出し順序に意味がある）。
- **描画への統合**: `EditorCanvas.drawLineWithFullWidthSupport()`に`List<SyntaxToken> tokens`引数を追加し、文字ごとに現在のトークンの`SyntaxKind`から`Theme`の対応色（`syntaxColor()`）を選んで描画するようにした。`isErrorLine`（F10/F11/F12の`*compile*`/`*run*`赤字表示）が最優先で、構文色より常に勝つ（既存のERROR_COLOR方式を変更していない）。DEFAULT色（=`theme.foreground`）の文字は従来通り本文専用グリフキャッシュ（`glyphCacheFg`、色固定で高速）を使い、それ以外の構文色は既存の汎用色付きグリフキャッシュ`uiGlyphCache`（telescope等が使っているものと同じ、`(codePoint,cellW,cellH,rgb)`キー）を流用する。新しいキャッシュ機構は追加していない。wrap（`:wrap`）時は行全体をトークン化してから`[segStart,segEnd)`でクリップし座標をセグメント相対へシフトする`tokensForSegment()`を使う（ごく長い1行の文字列リテラルが折返し境界をまたぐ場合、セグメント単独では開始前のコンテキストが失われ誤判定しうるが、既知の軽微な限界として許容した）。
- **`Theme`に`syntaxKeyword`/`syntaxType`/`syntaxString`/`syntaxComment`/`syntaxNumber`/`syntaxPreprocessor`の6フィールドを追加**。`syntaxKeyword`は参考画像上でキーワード（if/static/return等）が通常の識別子と同じ地の色で描画されていたため、値は`foreground`と同じにしている（本文フォントMiscFixedBold9x15が既にBoldのため、キーワードだけを別途太字にする追加の描画手段がなく、参考画像の実際の見た目とも一致するため、あえて色を分けなかった）。LIGHT_MODE側の値は参考画像に含まれないため、一般的なIDE配色（コメント=緑、文字列=暗い赤、型=ティール、数値=青、プリプロセッサ=紫）を独自に選定した（ユーザーからの指定なし、ダークモードがデフォルトになったため相対的に優先度が低い）。
- **デフォルトテーマをDARK_MODEに変更**: `Main.createLeaf()`の`canvas.setTheme(Theme.LIGHT_MODE)`を`Theme.DARK_MODE`に変更した。`EditorCanvas.theme`フィールド自体の初期値（`Theme.LIGHT_MODE`）は変更していない（既存テストの多くが`setTheme()`を明示的に呼んでおり依存していないため実害はないが、フィールドデフォルト自体を変える理由もなく、実際のアプリ起動経路である`Main.java`側だけを変更する方が変更範囲が最小になるため）。
- **意図的にスコープ外とした点**: Javaのtext block（`"""..."""`）は複数行文字列だが、`tokenizeLine()`は単一の`"`ペアとしてしか扱わないため、text block内部は通常の文字列走査ロジックの延長で（多くの場合）STRING扱いにならず誤判定しうる（既知の限界。ブロックコメントほど頻出しないため今回は対応しなかった）。エスケープを跨いだ文字列内の`//`/`/*`（文字列リテラル内のコメント様の文字列）は、文字列トークンを`"`〜`"`で1つのトークンとして先に確定させてから走査を再開するため誤判定しない。
- **テスト**: 新規`test/dev/javatexteditor/ui/SyntaxHighlighterTest.java`（キーワード/型/文字列/コメント（行・複数行）/数値/プリプロセッサ/PascalCase・ALL_CAPS識別子の分類、`computeBlockCommentStarts`の複数行ブロックコメント追跡を検証）。既存の`EditorCanvasTest`（51/51）・全体テストスイートは無修正で回帰なし（既知のベースラインFAIL＝`ScrollTest`2件・`ModalEditorTest`1件のみ、本変更前から存在）。

## 構文ハイライト配色の調整（背景を純黒に・記号/演算子の分離、2026-07-26追記）

上記「Java/C構文ハイライトの新規実装」の直後、実機での見た目確認を受けてユーザーから3点の追加調整依頼があり対応した。

- **DARK_MODEの背景を純黒(#000000)に変更**: `Theme.DARK_MODE.background`を`0x1A1A1A`から`0x00,0x00,0x00`に変更した。zz末尾超過領域の白黒塗り（本ファイル前出の節参照）は元々DARK_MODE時`Color.BLACK`固定だったため、これで通常領域と末尾超過領域の黒が完全に一致するようになった（副作用として視覚的な違和感が解消された）。
- **通常の文字色とキーワード色を分離**: 従来`syntaxKeyword`は`foreground`と同値だったが、「予約語は明るい白色に」という要望を受けて`foreground`（通常の文字、少し明るい灰色 `0xB8B8B8`）より明るい`0xF2F2F2`に変更した。
- **型名（`syntaxType`）を明るい水色に変更**: `0x6EC0C8`→`0x7FE0FF`。JDK/自作クラス問わず`SyntaxKind.TYPE`（PascalCase識別子・ALL_CAPS識別子・プリミティブ型キーワード）に一律適用される既存の分類ロジックはそのまま流用（クラスの由来がJDKか自プロジェクトかを区別する処理は行っていない。既存の`gr`/`:grep`等と同じヒューリスティックの範囲）。
- **`SyntaxKind`に`SYMBOL`（記号）・`OPERATOR`（演算子）を追加**: 従来、識別子・文字列・コメント・数値・プリプロセッサ以外の文字（括弧・カンマ・セミコロン・算術演算子等）はすべて`DEFAULT`（`foreground`と同色）で一括りにしていたが、「記号は明るい緑、演算子は少し暗い水色に」という要望を受けて分離した。`SyntaxHighlighter.classifyPunctuation(char)`が`OPERATOR_CHARS`（`+-*/%=<>!&|^~?`）・`SYMBOL_CHARS`（`(){}[];,.:@`）の2つの文字集合で判定する。複数文字演算子（`==`・`&&`・`->`等）も1文字ずつOPERATOR判定されるだけで、視覚的には隣接する同色トークンとして連続して見えるため、複数文字トークンとして特別扱いする実装はしていない（既存の1文字単位のDEFAULTトークン生成をそのまま演算子/記号判定に置き換えただけ）。
...
## 5万文字規模のJavaファイルで入力時に描画がカクつく不具合の調査・修正（2026-08-03）

- **報告**: mmap/Piece Table/CRLF・BOM対応（editor-buffer-architectureスキルの大容量ファイル対応）が完了済みで、mmap経路（8MiB以上で発動）の対象外である5万文字程度のファイルでも、入力時の再描画がカクつくという報告があった。
- **計測方法**: `test/dev/javatexteditor/performance/EditKeystrokePerfProbe.java`（使い捨て計測ハーネス。`VisualPreview.java`と同種でファイル名を意図的に`*Test.java`にしていないため`scripts/test.sh`の自動実行対象外）を新設した。約5万文字のJavaソース（`.java`パス付きで`ModalEditor`を構築し構文ハイライトを有効化）を開き、文書末尾（`G`）でINSERTモードに入り1文字入力するたびに`System.nanoTime()`で「`processKey`（`syncCanvas`込み）」と「実際の`paint`（`BufferedImage`への`canvas.paint()`呼び出し、`VisualPreview`と同じ手法でヘッドレスに`paintComponent`を強制実行）」を計測した。
- **計測結果（修正前）**: `refreshCanvasTextCache`（`buffer.getText()`+`split("\n",-1)`、ModalEditor.java）は1回あたり数十〜数百us、`paintContent`（既にビューポート分の行のみ描画。本SKILL冒頭のスコープ外事項参照）は3.5〜6.5msで文書サイズに応じて増加しない一定コストだったのに対し、`EditorCanvas.setText()`内で呼ばれる`SyntaxHighlighter.computeBlockCommentStarts()`（構文ハイライトのブロックコメント状態を文書全体について`tokenizeLine()`し直す処理）が2〜10ms（1181行）と最も支配的だった。`processKey`（`syncCanvas`込み）合計6.5〜18msのうち、このメソッド1つで大半を占めていた。
- **根本原因**: `EditorCanvas.setText(text, lines)`は、渡された`lines`配列の**参照**が前回と異なる場合にのみ`computeBlockCommentStarts()`を呼ぶガードを持っていた（「telescope・ステータス行…」節を参照）。しかし`ModalEditor.refreshCanvasTextCache()`は実際の編集があるたびに`buffer.getText().split("\n", -1)`で**必ず新しい配列インスタンス**を生成するため、たとえ1文字の入力であってもこのガードは毎回「変化あり」と判定し、文書の**全行**を`tokenizeLine()`で再走査していた。これは「1キー入力のたびに文書全体に対して全行を返すメソッドが呼ばれている」パターンの一種（`buffer.getText()`自体ではなく、そこから派生した`lines`配列を消費する側の構文ハイライトキャッシュで発生していた）。
- **`buffer.getText()`自体は無駄だが軽微**: 副次的に判明した点として、`ModalEditor.refreshCanvasTextCache()`が生成する`canvasCachedText`（`buffer.getText()`の結果、文字列全体）は`canvas.setText(canvasCachedText, canvasCachedLines)`経由で`EditorCanvas.text`フィールドに格納されるが、**このフィールドは描画（`paintContent`等）で一切読み取られていない**（描画は常に行配列`cachedLines`のみを使う。`this.text`への代入以外に読み出し箇所が存在しないことをコードベース全体のgrepで確認済み）。5万文字規模では`buffer.getText()`のコストは数十〜数百usと軽微なため今回は変更していないが、将来的にさらに大きなファイルで再度この経路がボトルネックとして疑われた場合、`canvas.text`フィールド自体を削除して`buffer.getText()`の呼び出しを省略できる可能性がある（未対応・スコープ外）。
- **採用した修正（データ生成側・部分再計算）**: `SyntaxHighlighter`に`computeBlockCommentStartsIncremental(oldLines, oldStarts, newLines, lang)`を追加し、`EditorCanvas.setText()`から呼ぶようにした。Piece Table側に「変更行範囲を返す差分API」を追加する案（要望文中の対応方針a）ではなく、既に`lines`配列という文字列の集合を持っているEditorCanvas側で新旧配列を直接比較する方式を採った（PieceTable自体には変更なし。60箇所以上ある`canvasCachedLines`消費側にも一切手を入れていない）。
  - **アルゴリズム**: (1) 新旧`lines`配列を先頭から`String#equals()`で比較し、実際に内容が異なる最初の行`firstDiff`を確定する（当て推量ではなく実際の内容比較による確定）。(2) `firstDiff`から`tokenizeLine()`を前方に再実行し、各行で「新旧の行内容が一致し、かつ新たに計算した終端状態が旧キャッシュの次行開始状態と一致する」ことを確認できた時点（収束点）で打ち切り、以降は旧`oldStarts`をそのまま流用する。(3) 行数が変化する編集（改行の挿入・削除、複数行貼り付け等）は新旧配列の長さが一致しないため、安全側に倒して`computeBlockCommentStarts()`（全文再計算）にフォールバックする。
  - **正しさの保証**: 呼び出し元（`ModalEditor`）から「どの行が変わったか」というヒントは一切受け取らない。`firstDiff`も収束判定も実際の`String#equals()`比較に基づいて自己検証しており、外部からの当て推量に依存しない。そのため`ModalEditor`側の76箇所ある`buffer.insert()`/`buffer.delete()`呼び出し元（キー入力・マクロ再生・置換・リファクタリング・undo/redo等）のいずれも変更不要で、`EditorCanvas`内部だけで完結する。
  - **計算量（変更前後の比較）**: 変更前は1キー入力ごとに常にO(文書行数)の`tokenizeLine()`（`List`/`SyntaxToken` record確保を伴う相対的に重い処理）。変更後は`firstDiff`探索のための`String#equals()`比較が最悪でもO(文書行数)だが、比較コスト自体がアロケーション無しで`tokenizeLine()`よりはるかに軽いため、実測では1181行の文書で2〜10ms→数十〜数百usまで縮小した（末尾の1行だけを編集する、行数が変わらない単純な1文字編集で計測。詳細は`EditKeystrokePerfProbe`のJavadoc参照）。ブロックコメントの開始/終了など複数行にまたがる状態変化を伴う編集では、変化が波及した範囲までは正しく再計算され続ける（収束するまで打ち切らない）ため、そのようなケースでは短縮効果は小さくなるが、正しさは常に維持される。
  - **検討したが採用しなかった代替案**: (a) `ModalEditor`側で「編集した行番号」のヒントを`EditorCanvas`へ渡す方式は、`buffer.insert()`/`buffer.delete()`呼び出し元が76箇所あり、ヒントの伝搬漏れが1箇所でもあれば構文ハイライトの状態が黙って不正になる（描画バグ）リスクがあり、CLAUDE.mdの「既存の60箇所以上の呼び出し箇所の挙動を変更しない」という要件とも相容れないため見送った。(b) Piece Tableに「変更行範囲を返す差分API」を追加する案（要望文中の方針a）は、実測の結果ボトルネックが`buffer.getText()`/`split()`ではなく構文ハイライト側にあることが判明したため、対象外とした。
- **テスト**: `test/dev/javatexteditor/ui/SyntaxHighlighterTest.java`に5ケース追加（計49/49）。単純な1行編集・ブロックコメント開始行の編集（後続行への波及）・末尾行のみの編集・行数変化時のフォールバック・初回（旧情報無し）のいずれについても、`computeBlockCommentStartsIncremental()`の結果が`computeBlockCommentStarts()`（全文再計算、正解の基準）と完全に一致することを検証している。既存の`EditorCanvasTest`（51/51）・全体テストスイートは無修正で回帰なし（既知のベースラインFAIL＝`ScrollTest`のhalfPageUp系2件・`ModalEditorTest`1件・`FilerTest`1件のみ。`FilerTest`の1件は本修正のブランチ作成前から存在する既存の不安定挙動であることを、変更を`git stash`で退避した状態で同一テストを実行し確認済み）。

## フォントサイズを上げるとカーソル移動・スクロールの描画が滑らかでなくなる不具合の調査・修正（2026-08-03続報）

- **報告**: 文書サイズ・行数は変えずフォントサイズ（`EditorCanvas.cellW`/`cellH`）だけを大きくすると、カーソル移動・スクロールの描画がなめらかでなくなる。前節（5万文字ファイルの入力カクつき）と異なり「文書サイズ」ではなく「フォントサイズ」に依存する点が新しい。
- **計測方法**: `test/dev/javatexteditor/performance/FontSizeRenderPerfProbe.java`（使い捨て計測ハーネス。前節の`EditKeystrokePerfProbe.java`と同種で`scripts/test.sh`の自動実行対象外）を新設し、以下3点を個別に計測した。
  - a) `EditorCanvas.xForCol()`（列番号→Xピクセル座標変換）をリフレクションで直接呼び、charWidth（フォントサイズ相当）と列番号を独立に振って計測。
  - b) 1グリフあたりの純粋な描画コスト。paintContent全体（可視セル数がフォントサイズで変動し交絡するため参考値）に加え、`g2.drawString()`と`g2.drawImage()`（キャッシュ済みBufferedImage想定）それぞれ単体を、可視グリフ枚数を2000枚に固定した状態で比較。
  - c) `EditorCanvas.requestRepaint()`の実装（コードを直接確認）。
- **計測結果**:
  - a) `xForCol`は charWidth（9px/18px/36px）を変えても同じ列番号に対する所要時間はほぼ一定で、列番号が大きいほど線形に増加した（例: charWidth=36pxで col=10→0.2us、col=19999→4.5us）。**フォントサイズではなく列番号に比例**しており、今回の症状の直接要因ではない。ただし呼び出し元は行頭から毎回線形走査する実装のままであり、非常に長い行を持つ文書では別途O(col)コストが残る一般的非効率として記録しておく（今回は対応方針で「a)が原因の場合」に該当しなかったため未修正）。
  - b) 固定1400x900ウィンドウでの`paintContent`全体時間は、フォントサイズを上げても単調増加しなかった（可視セル数がフォントサイズに反比例して減るため相殺される。既存の「paintContentは既にビューポート限定描画」という前提と整合する）。一方、可視グリフ枚数を固定した単体比較では、`g2.drawString()`（非ASCIIフォールバック経路相当）は cellH=15→30→60px で 1.2us→1.7us→3.6us/文字、`g2.drawImage()`（ASCII本文のキャッシュ済みビットマップ経路相当）は cellW=9→18→36px で 0.5us→1.2us→2.5us/文字と、**両方ともフォントサイズにほぼ比例して増加**した。この「1グリフあたりのコストがフォントサイズに比例する」という物理的性質（ピクセル面積に比例するアンチエイリアス合成コスト）自体はキャッシュの有無に関わらず避けられないが、`drawString`は`drawImage`よりおおむね1.5〜3倍高いコストで、かつ**修正前は本文中の非ASCII文字（日本語コメント等）についてこの`drawString`を毎paintごとにキャッシュせず直接呼んでいた**（ASCII文字は`glyphCacheFg`/`glyphCacheBg`で1回描画してBufferedImageとしてキャッシュ済みだったのに対し、`MiscFixedBold9x15`が非対応の文字＝日本語等はキャッシュ機構が存在しなかった）。本プロジェクト自身のソースのように日本語コメントを多く含む典型的な文書では、可視行全ての日本語文字が毎フレーム未キャッシュの`drawString`を通っており、フォントサイズが大きくなるほど1文字あたりのコスト差（drawStringの方が高コスト）がそのまま累積する構造だった。
  - c) `EditorCanvas.requestRepaint()`は常に引数なし`repaint()`を呼ぶ実装で、カーソル移動・スクロールのたびにビューポート全体が再描画対象になる。ただし`paintContent`自体が既にビューポート限定描画のため、この「全体」は文書サイズに依存せず、b)の計測結果と合わせて判断する限り今回の症状の支配要因ではないと判断した（対応方針で「c)が原因の場合」には該当しないため未修正）。
- **採用した修正（b)：非ASCII文字グリフのキャッシュ化）**: ASCII本文と同じ「1回だけBufferedImageへ描画し、以降は`drawImage()`で使い回す」方式を、`drawLineWithFullWidthSupport()`の非ASCIIフォールバック分岐（旧: 毎paintで`g2.drawString()`を直接呼んでいた箇所）に適用した。
  - 新設フィールド`nonAsciiGlyphCache`（`Map<UiGlyphKey, BufferedImage>`、既存の`uiGlyphCache`と同じ`UiGlyphKey(codePoint, cellW, cellH, rgb)`をキーに再利用。非ASCII文字のcodePointは`bitmapFont.isSupported()`の範囲(0x20-0x7E)と重ならないためASCII用`uiGlyphCache`とのキー衝突は起きないが、意味的に別キャッシュのため独立したMapにした）。
  - 新設メソッド`getNonAsciiGlyph()`/`renderNonAsciiGlyph()`（既存の`getUiGlyph()`と対になる構造。`getSwingFont()`で`g2.drawString()`していた処理をオフスクリーンの小さな`BufferedImage`へ1回だけ描画する処理に置き換え、以降は同じ画像を`drawImage()`で使い回す）。
  - 既存の`invalidateGlyphCache()`（セルサイズ変更・テーマ変更・フォント切替時に呼ばれる）に`nonAsciiGlyphCache.clear()`を追加し、他のグリフキャッシュと同じタイミングで無効化されるようにした。
  - `drawLineWithFullWidthSupport()`内で使われなくなった`swingBaselineY`ローカル変数（旧`drawString`呼び出し専用）を削除した。
  - **計算量（変更前後の比較）**: 変更前は文書中に出現する非ASCII文字の**可視インスタンス数**に比例して毎paintごとに`drawString`（フォントレイアウト・アンチエイリアス合成込みのラスタライズ）が走っていた。変更後は文書中に出現する**ユニークな(コードポイント,セルサイズ,色)の組み合わせ数**だけ初回コストがかかり、以降は同じ組み合わせの再描画は`drawImage`（キャッシュ済みビットマップの単純合成）に置き換わる。実測ではb-2)の単体比較で`drawImage`は`drawString`よりおおむね1.5〜3倍安く、かつ日本語コメント中の漢字・かなは同じ語が高頻度で繰り返されるため（本プロジェクトの`// ○番目のフィールドを説明する日本語のコメント行です`のような定型文が典型例）、実文書ではキャッシュヒット率が高くなりやすい。ただし`paintContent`全体で見た改善幅は、他の描画処理（トークナイズ・ガター・カーソル・選択範囲等）や、フォントサイズが大きいほど可視セル数自体が減るという相殺効果に埋もれて数値上明確には出なかった（b)の`paintContent`計測値を参照）。そのため本修正は「測定可能な劇的な高速化」ではなく、「ASCII本文とアーキテクチャ上一貫させ、可視行数・文字種の多い実文書で無駄なラスタライズが累積する構造を除去する」正しさ・一貫性重視の修正と位置づける。
  - **検討したが採用しなかった対応方針**: a)（xForColの列キャッシュ化）・c)（repaint矩形の最小化）はいずれも計測の結果、今回の症状の支配的要因ではないと判断されたため見送った。特にc)は、`paintContent`自体が可視カーソルセルだけでなくビューポート全体を毎回再構築する既存設計（本SKILL冒頭のv1スコープ・future-phases.md参照）を前提にしており、repaint矩形だけを最小化してもpaintContent内部の描画量は変わらないため、単独では効果が薄いと判断した（差分描画自体を導入するのは既存アーキテクチャへの大きな変更を伴うため、CLAUDE.mdの「推測だけで広範囲を書き換えない」という要件からスコープ外とした）。
  - **計測の限界（正直な注記）**: 今回のヘッドレス計測（オフスクリーン`BufferedImage`への`canvas.paint()`）では、固定ウィンドウサイズという条件下では`paintContent`全体の所要時間はフォントサイズを上げても劇的には悪化しなかった（可視セル数の減少と1グリフあたりコストの増加が相殺するため）。実機での「滑らかでなくなる」という体感は、ウィンドウを最大化した状態でフォントサイズだけ上げて可視セル数があまり減らない使い方や、実際のディスプレイ・ウィンドウシステム側のコンポジット処理（本ハーネスでは再現できない）に起因している可能性があり、今回のb)修正はこの体感を完全に解消することを保証するものではない。追加の体感改善が必要な場合は、実際のGUIウィンドウでの実機計測（本SKILL「実機検証で判明した修正点」節と同様の手順）を追加で行うことを推奨する。
- **テスト**: `test/dev/javatexteditor/ui/NonAsciiGlyphCacheTest.java`を新設（5/5 PASS）。(1)非ASCII文字がLIGHT_MODE前景色で描画される、(2)テーマ変更後にDARK_MODE前景色へ切り替わる（`invalidateGlyphCache()`が`nonAsciiGlyphCache`もクリアしていることの確認。アンチエイリアスの縁は前景/背景の中間色になり厳密な色一致では偶然の誤検出が起きたため、明度の大小関係で判定する方式に修正した）、(3)フォントサイズ変更後に描画結果のサイズが変わる（旧セルサイズのキャッシュ画像を使い回していないことの確認）、(4)同一文字の連続描画結果が完全一致する（キャッシュ再利用の一貫性）、(5)全角文字を含む行でカーソルXピクセル座標が等幅計算(列×セル幅)と一致する、を検証している。全体テストスイート（`scripts/test.sh`）は本修正前後で結果に変化なし（既知のベースラインFAIL＝`ScrollTest`のhalfPageUp系2件・`ModalEditorTest`1件・`FilerTest`1件のみ。`git stash`で本修正を退避した状態で同一テストを実行し、これら3件が本修正と無関係の既存事象であることを確認済み）。

## アイドル時に描画経路がゴミを出し続けていた問題の修正（2026-08-07）

エディタを操作せず放置しているだけでもヒープが増え続ける問題の調査で、30fpsの
`animTimer`（ステータスライン再描画）まわりに「キャッシュヒット時でも毎回割り当てが起きる」
箇所が複数見つかった。JFR（`settings=profile`）のアイドル時アロケーションプロファイルで
上位に現れたものを、**描画結果を1ピクセルも変えない範囲で**取り除いた。

| 箇所 | 何が毎フレーム割り当てられていたか | 修正 |
|---|---|---|
| `WalkingPersonSprite.drawFrame` | 半透明ピクセルごとの `new Color(br,bg,bb,178/102)`（最大256回/フレーム） | 前景色ごとに1組だけ作って使い回す（`ShadeColors` レコードのキャッシュ） |
| `EditorCanvas.getUiGlyph` / `getNonAsciiGlyph` / `getGlyphFg` / `getGlyphBg` | `Map.computeIfAbsent` に渡す**キャプチャ付きラムダ**（キャッシュヒット時も毎回生成） | `get()` → null なら `put()` の素朴な形へ書き換え |
| `EditorCanvas` のタイマー分解能ピン留めスレッド | 全プラットフォームで `Thread.sleep(1)` を毎秒1000回。効果があるのはWindowsのみ | `os.name` が Windows のときだけ起動する |
| `SystemStatsMonitor`（2秒ごと） | `nvidia-smi` が無い環境でも永久にプロセス起動を試行し、失敗のたびに `IOException` を生成 | 起動失敗（＝実行ファイルが無い）を検知したら以後は試さない |
| `SystemStatsMonitor`（2秒ごと） | `/proc/meminfo` の `Files.readAllLines`（8KB文字バッファ＋全行の `String`）と行ごとの正規表現 `Matcher` | 使い回しの `byte[]` へ読み込み、`MemTotal`/`MemAvailable` をバイト走査で直接パースする |

### スプライト描画で採らなかった案（どちらも描画結果が変わることを実測で確認）

歩行キャラクターは「1ピクセル1回の `fillRect`」を拡大変換つきの `Graphics2D` に対して行っている。
これを軽くする自然な案は2つあるが、いずれもピクセル単位で結果が変わるため採用しなかった。

1. **`BufferedImage` に1度だけラスタライズして `drawImage` で貼る** —
   透明画像へ描いてから合成すると8bit量子化が2回かかり、半透明ピクセルが直接合成した場合と
   最大1/255ずれる（実測: 71ピクセルが各チャンネル±1）。
2. **同じ文字が連続する区間を1回の `fillRect` にまとめる** —
   拡大変換下では「幅Nの矩形1つ」と「幅1の矩形N個」でJava2D側の丸めが一致せず、
   輪郭が目に見えて変わる（実測: 87ピクセル・最大チャンネル差48）。

そのため描画経路自体は従来のまま残し、`Color` の生成だけをキャッシュに置き換えている。
検証は「変更前の `drawFrame` をそのまま複製した参照実装」と新実装を、
文字高8〜150px × 3色 × 2フレーム × 3つのX座標（計144通り）で全ピクセル比較し、差分0を確認した。
