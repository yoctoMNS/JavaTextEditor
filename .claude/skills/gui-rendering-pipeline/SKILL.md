...
## マルチバイト文字の重なり不具合・第2弾（環境依存文字への対応 ＋ 描画クリップによる構造的な重なり防止）

上記「`charCellWidth()`のUnicode範囲漏れ〜」節の修正後、「環境依存文字にも対応し、まだ重なる可能性がある文字が無いか調査した上で、全ての文字が重ならないよう実装してほしい」という追加要望があった。単発のUnicode範囲追加では「次に見つかる未知の文字」への対応が原理的に終わらない（Unicodeは約15万コードポイントあり、静的な範囲テーブルでの網羅には限界がある）ため、今回は**判定テーブルの拡充**と**描画側の構造的な安全策**の2段構えで対応した。

- **1段目: `charCellWidth()`のWIDE_RANGESテーブルを拡充し、環境依存文字（機種依存文字）を明示的に網羅した**。日本語IT用語の「環境依存文字」は主にShift_JISのNEC選定IBM拡張文字・IBM拡張文字を指し、具体的には丸数字(①②③)・ローマ数字(Ⅰ Ⅱ Ⅲ)・単位記号(㎜㎝㎞)・囲みCJK文字(㈱㈲㈹)等が該当する。これに加え、日本語の文章・箇条書きで一般的に全角幅で描画される幾何学模様(■□▲△▼▽◆◇○●)・記号(☆★)等も同時に追加した（一つ前の節で報告された「■これは統一感のある...」のような箇条書きの重なりは、まさにこの■(U+25A0)が判定範囲から漏れていたことが原因だった）。追加した範囲: 数字に準じる記号・ローマ数字(0x2150-0x218F)、囲み英数字(0x2460-0x24FF)、幾何学模様・記号・装飾記号(0x25A0-0x27BF)、CJK部首補助(0x2E80-0x2EFF)、康熙部首(0x2F00-0x2FDF)、囲みCJK文字・月(0x3200-0x32FF)、CJK互換用文字(0x3300-0x33FF)、CJK統合漢字拡張A(0x3400-0x4DBF)、ハングル音節(0xAC00-0xD7A3)、CJK互換漢字(0xF900-0xFAFF)、CJK互換形記号(0xFE30-0xFE4F)、CJK統合漢字拡張B〜F等の補助漢字面(0x20000-0x2FFFF、サロゲートペア)。範囲が増えたため、実装は従来の逐次`if`列挙から`WIDE_RANGES`という`int[][]`テーブル＋ループ判定に変更した（保守性のため。件数が少ない間は逐次ifで十分だったが、15件を超えたため一覧性を優先した）。
- **2段目（本質的な対策）: 非ASCIIフォールバック（Swingフォント）の描画を、割り当てられたセル矩形に強制的にクリップする`drawClippedSwingChar()`を新設した**。1段目のテーブル拡充だけでは「テーブルに載っていない文字」「テーブルの分類は正しくても実際のフォントが描画するグリフがセル幅を超える場合（絵文字・合字・特殊フォントの互換分解表示等）」に対して原理的に無力（新しい重なりが将来また見つかる可能性が残る）。`drawClippedSwingChar()`は`g2.create(cellX, cellTopY, cellPixelWidth, cellPixelHeight)`で親のGraphics2Dから「その文字に割り当てられたセル矩形だけに移動・クリップされた」子Graphics2Dを作り、その中でのみ`drawString()`を呼ぶ。`Graphics.create(x,y,w,h)`は平行移動とクリップ領域の設定を行うため、実際のフォントがどれだけ大きく描画しようとしても、割り当てられたセルの外にピクセルが一切漏れなくなる。これにより「`charCellWidth()`の判定漏れ・想定外のフォント合成」に対しても、**重なりが起きないことを構造的に保証**できる（テーブルの精度に依存しない最終防波堤）。
  - **適用箇所**: `drawLineWithFullWidthSupport()`（本文）・`drawCursor()`（カーソル下の文字を背景色で再描画する箇所）・`drawUiText()`（telescope・ステータス行・補完ポップアップ等、本文以外のUI文字列。共有ヘルパーのため1箇所の修正で全UI要素に自動的に反映される）の3箇所。ASCII文字（`TtfMonoFont`でBufferedImageに事前ラスタライズ済み）はそもそも画像サイズ自体がセルぴったりのため、この対策は不要（既に安全）。
  - **なぜテーブル拡充だけで終わらせなかったか**: ユーザーからの要求が「全ての文字が重ならないように」という無条件の保証だったため。Unicode East Asian Width（UAX #11）の"Ambiguous"文字の扱いはフォント依存でもあり、静的テーブルだけで完全網羅したと主張することはできない。クリップによる構造的対策を加えることで、テーブルに何が抜けていようと「重ならない」という結果だけは常に保証される設計にした。
- **回帰テストで実際に検証した**: `drawClippedSwingChar()`を一時的に無効化（クリップなしの旧実装に戻す）した状態でテストを実行し、新設した「クリップ検証」テスト（後述）が実際に失敗する（`FAIL`、隣接セルの背景色部分にフォアグラウンド色のインクが漏れて検出される）ことを確認してから、クリップを復元した。単に「通ったから正しい」ではなく「直さなければ落ちる」ことまで確認済み。
- **テスト**: `test/dev/javatexteditor/ui/EditorCanvasTest.java`にTest 5c・5dを追加（計39テスト）。
  - Test 5c: `charCellWidth()`が丸数字(①)・ローマ数字(Ⅰ)・幾何学模様(■)・記号(☆)・囲みCJK文字(㈱)・CJK互換用文字(㎜)・CJK互換漢字・ハングル音節を全角(2)と判定することを検証。
  - Test 5d: `setInitialCellSize(5, 80)`で意図的に極端なセル比率（幅5px・高さ80px）を作り、Swingフォールバックフォントが`cellH*0.75=60pt`という「割り当てセル幅(10px)を大きく超えるサイズ」になる状況を作った上で、ひらがな「あ」を描画してもセル境界から十分外側(x=20)のピクセルが背景色のまま（グリフのインクが漏れていない）ことを検証する。上記の通り、クリップ実装を外すと実際に`FAIL`することを手動確認済み。
  - 目視確認: `「1あ」`・`■`箇条書き・`①②③ Ⅰ Ⅱ Ⅲ`・`㈱㈲㈹ ㎜㎝㎞`・`☆★○●◎`を含むテキストを`EditorCanvas`に描画しPNG出力して確認し、いずれも重なりが解消されていることを確認した。

## マルチバイト文字の重なり不具合・第3弾（矢印・罫線の判定漏れ）

第2弾のWIDE_RANGES拡充後、「矢印や罫線の記号はまだ重なってしまう」という追加報告があった。

- **原因**: 第2弾で追加した`{0x2150, 0x218F}`（数字に準じる記号・ローマ数字）→`{0x2460, 0x24FF}`（囲み英数字）→`{0x25A0, 0x27BF}`（幾何学模様等）という3つのレンジの**間に隙間があり**、そこに矢印(Arrows, U+2190-21FF)・数学記号(Mathematical Operators, U+2200-22FF)・その他の技術用記号(Miscellaneous Technical, U+2300-23FF)・制御記号(Control Pictures, U+2400-243F)・OCR記号(U+2440-245F)に加えて、**罫線(Box Drawing, U+2500-257F)・ブロック要素(Block Elements, U+2580-259F)**が丸ごと判定漏れしていた（`0x218F`の次が`0x2460`、`0x24FF`の次が`0x25A0`という飛び方をしており、矢印(2190番台)は前者の隙間、罫線(2500番台)は後者の隙間に落ちていた）。第2弾は「個別に見つかった記号を都度追加する」進め方をしたため、この種の隙間を生みやすい構造的な問題があった。
- **修正方針の転換**: 個別レンジの追加をこれ以上繰り返すと同じ問題が再発し続けると判断し、`{0x2150,0x218F}`・`{0x2460,0x24FF}`・`{0x25A0,0x27BF}`の3レンジを**`{0x2100, 0x27FF}`という1つの連続レンジに統合**した。これはLetterlike Symbols(℃№等)からSupplemental Arrows-A（矢印の拡張）まで、記号・矢印・数学記号系のブロックが隙間なく連続している領域であり、個別のブロック境界を意識せず丸ごと全角として扱うことで「次に見つかる漏れ」を構造的に無くした。あわせて`{0x2900, 0x2BFF}`（Supplemental Arrows-B・その他の数学記号・その他の記号と矢印）も追加し、矢印系の拡張ブロックも網羅した。
- **Braille Patterns(U+2800-28FF)を意図的に除外**: 上記の連続統合の範囲(`0x2100-0x27FF`と`0x2900-0x2BFF`)は、ちょうどBraille Patterns(0x2800-0x28FF)を挟む形になっている。点字は東アジアの文脈でも常に半角(Neutral)として扱われるのがUnicode東アジア幅特性上の一般的な扱いであり、日本語フォントでも全角化される慣習が無いため、意図的にこの区間だけ空けている。
- **教訓（次にこの表を触る開発者へ）**: この不具合は「個別のブロックを都度追加する」やり方の限界を示している。Unicodeの記号領域(U+2000番台〜U+2BFF番台)は意味的に連続したブロック群であり、一部だけ拾うと必ずどこかに隙間ができる。新しい記号の重なりが報告された場合、まず「その文字が属するブロックの前後にまだ拾えていない隣接ブロックが無いか」を確認し、個別のcodepointだけでなく**ブロック単位・領域単位**で判定漏れがないかを疑うこと。なお、第2弾で導入した`drawClippedSwingChar()`によるクリップは今回の矢印・罫線の重なりに対しても引き続き機能しており（クリップ自体は文字種によらない汎用の安全策のため）、判定漏れがあっても「文字が隣のセルの外へはみ出て重なる」という最悪の事態そのものは第2弾の時点で既に防がれていた。ただし、はみ出さなくても「本来2セル分の見た目を持つはずの記号が1セル分に押し込められて窮屈に見える」という表示品質の問題は解消されないため、判定テーブル自体の精度向上は引き続き重要である。
- **テスト**: `test/dev/javatexteditor/ui/EditorCanvasTest.java`にTest 5c-2を追加（計40テスト）。矢印(→←↑↓)・罫線(─│┌)・ブロック要素(█)・数学記号(∀)・Letterlike Symbols(℃№)・Supplemental Arrows-B(⤓)が全角(2)、Braille Patterns(⣿)が引き続き半角(1)のままであることを検証。
- **目視確認**: `→ ← ↑ ↓ ↔`・`─│┌┐└┘├┤┬┴┼`・`█▓▒░ ▀▄`・`℃ № ∀ ∃`を含むテキストを描画しPNG出力して確認し、いずれも重なりが解消されていることを確認した（一部の記号はヘッドレス環境のフォールバックフォントに専用グリフが無く、見た目上別の文字に置き換わって表示される場合があるが、これはフォント側のグリフ欠落によるものであり、幅判定・重なり防止とは別種の既知の制約）。
