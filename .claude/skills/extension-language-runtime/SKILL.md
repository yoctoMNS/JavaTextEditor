...
2. **クラスパスの継承**：プラグインが `dev.vimacs.*` の型を使えるように、コンパイル時に
...
4. **クラス名の決定**：ソースファイル名（拡張子除く）をクラス名として使う。
   パッケージ宣言がある場合は `package dev.vimacs.plugins;` を先頭に付けることを規約化する。
...
| ファイル | 役割 |
|---|---|
| `src/dev/vimacs/extension/EditorContext.java` | プラグイン向けエディタ操作インタフェース |
| `src/dev/vimacs/extension/EditorPlugin.java` | プラグイン実装インタフェース |
| `src/dev/vimacs/extension/PluginLoadException.java` | コンパイル失敗・型不整合エラー |
| `src/dev/vimacs/extension/PluginLoader.java` | 動的コンパイル・ロード・ライフサイクル管理 |
| `src/dev/vimacs/extension/SimpleEditorContext.java` | ModalEditor をラップした EditorContext 実装 |
| `test/dev/vimacs/extension/PluginLoaderTest.java` | 自作テストハーネス（5+ ケース） |
