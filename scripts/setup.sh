...
# :font 1 で選択できる半角ASCIIフォント（既定の :font 0 は MiscFixedBold9x15.java に
# ビットマップとして直接埋め込み済みのためダウンロード不要）。SIL OFL 1.1 で配布されており、
# ソース同梱ではなく実行時にダウンロードして lib/fonts/ に置く（lib/ は .gitignore 対象の
# ため、src.zip/openjdk-native と同じ「外部リソースは setup.sh で取得」という既存の方針に
# 合わせている）。
