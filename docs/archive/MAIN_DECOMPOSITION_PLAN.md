...
（CLAUDE.md「既知の未接続・二重定義」6.／`docs/archive/REFACTORING_PLAN.md` U-7）。
...
| 5 | 2026-07-27 | （本コミット） | ✅ | ✅ 124 | 634 → **625**（−9） | ① **計画のスケッチから変更**: `main()` 内で生成する案ではなく `private static final` フィールドにした。旧 `JDK_INDEX` はクラスロード時に構築を開始しており、static final にすればタイミングが厳密に一致するため（`main()` 内へ移すとわずかに遅れる）。② **`WD_MANAGER`/`BUFFER_REGISTRY` は含めなかった**。解析サービスではなく、名前に無関係なものを詰めると SRP 違反を再生産するため。③ `clinit` の逆アセンブルで `JdkClassIndex.build()` の呼び出し位置が変更前と同じ10番目であることを確認。④ 索引の実利用を実測（JDK索引462ms で `String`→`java.lang.String` を解決、`wireInto` 後に Alt+/ 補完が有効）。⑤ 削減が計画見込み（−35）より小さい（−9）のは、`createLeaf` の4配線を `wireInto` 1行にまとめた分と、新設した詳細コメントが相殺したため |
| 6 | 2026-07-28 | 6-0〜6-5（`136c00f`〜、本ブランチ） | ✅ | ✅ 124 | 625 → **343**（−282、6-1で達成。以降 `Main.java` は無変更） | 段階6を `docs/archive/STAGE6_OPTION_C_PLAN.md` で6-0〜6-5に細分化して実行。①6-1で `PaneManager` を新設し `root[0]`/`active[0]` の「箱」を解消（機械的移設のみでは `Main` がビルド不能と判明したため、計画時の6-1/6-2を統合して実施）。②6-3で `EditorHost` を新設（追加のみ）。③6-4で `ModalEditor.setHost(EditorHost)` を新設（8個のsetterへ委譲する糖衣構文、追加のみ）。④6-5で `PaneManager.refreshCallbacks()` を `setHost(this)` 1行へ統一 — **この配線切り替えで `:q` がペインを閉じなくなる重大な回帰が発生し、97テストクラス全ての `diff` では検出されなかった**（`ModalEditor.closeCurrentPane()` が `closeBlockedCallback != null` をnull判定そのもので分岐に使っており、`setHost()` が非null no-opを配線したため）。Xvfb+Robotによる手動ペイン検証で発見し、`setHost()` から `setCloseBlockedCallback` の配線を除外して修正。詳細・教訓は同計画書参照 |
| 7 | 2026-07-28 | 7-0〜7-2（`3a0675e`〜、本ブランチ） | ✅ | ✅ 124 | 343 → **15**（7-1で343→237、7-2で237→15） | 段階7を `docs/archive/STAGE7_PLAN.md` で7-0〜7-2に細分化して実行（段階6と同じ「1サブ段階=1コミット」原則）。①7-1で `GlobalKeyDispatcher` を新設し、`boolean[] pressedHandled` の「箱」をインスタンスフィールドへ変換（段階6の `root[0]`/`active[0]` と同種の箱だが、単一インスタンスしか登録されないため配列自体が不要という単純な変換で済んだ）。②7-2で `EditorApplication` を新設し `main()` の残り全部（引数解析・サービス生成・画面計測・GUI組み立て）を移設、`Main.java` を「`EditorApplication.launch(args)` を呼ぶだけ」の最終形にした。親計画書の `StartupArgs` 抽象は採用せず、段階5で確立済みの「static final フィールドとしてサービスを保持しクラスロード時に構築を開始する」方式をそのまま延長した（`javap -c -p` の `<clinit>` で初期化順序が保たれていることを確認）。③手動検証で「Escapeキー1回ではINSERTモードを抜けられない」という事象に遭遇したが、`git stash` でリファクタ前のバイナリと比較し**リファクタと無関係の既存挙動**（補完ポップアップが開いている間はEscapeがポップアップを閉じるだけで2回目のEscapeでINSERTを抜ける仕様）と判明、テスト手順を「Escape2回」に修正して対応した。④IME確認は実IMEの無いXvfb環境のため、`PaneManager.createLeaf()` が実際に配線する `canvas.setImeCommitHandler` のラムダ本体を再現しつつ `InputMethodEvent` を直接 `canvas.inputMethodTextChanged()` に渡す方式で実施し、「あ」の確定で1文字だけ挿入されること・この経路が `GlobalKeyDispatcher`/`pressedHandled` と独立していることを確認した。詳細・スクリーンショットの目視確認内容は同計画書参照 |
