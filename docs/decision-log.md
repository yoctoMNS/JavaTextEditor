...
## 起動時JVMメモリ/GCチューニングの設計決定事項（2026-08-04）

- **症状**: 実行中のエディタ（`scripts/run.sh`/`run.bat`起動）が、システムモニタ/htopで
  RES 9〜10GiB前後を占有したまま下がらず、さらにhtopのスレッド（LWP）行がPID列に連番で
  70件以上並ぶ（VIRT/RESが全行で同一＝同一プロセスの全スレッドを個別行表示するhtopの仕様）。
  ユーザーからは「大量のプロセスが増え続けている」ように見えると報告された。
- **調査した候補と切り分け結果**:
  1. **F10/F11自己実行によるJavaプロセス増殖**（既知の別バグ、2026-08に3フェーズで修正済み・
     `RunningProcessHolder.terminateAndStart`で排他制御済み）: `RunningProcessHolder`/
     `JavaBuildRunner`/`ProcessOutputPump`を再確認したが、多重実行防止が正しく機能しており
     新たな漏れは見つからなかった。今回の症状はこれとは別の原因と判断。
  2. **バックグラウンドスレッドのリーク**（`SystemStatsMonitor`・`LiveDiagnostics`・
     `ProjectSearcher`等）: いずれも仮想スレッド（`Thread.ofVirtual()`）または単一の
     `ScheduledExecutorService`（daemonスレッド1本）で、呼び出しごとに新規プラットフォーム
     スレッドを積み上げる実装は無かった。仮想スレッドはOSスレッドを直接消費しないため、
     htopで見えるスレッド数の説明にはならない。
  3. **無制限に肥大化するキャッシュ**（`JdkClassIndex`・`WordIndex`・`EditorCanvas`の
     グリフキャッシュ等）: `JdkClassIndex`はJDKの単純名→FQN文字列のみを保持（大きくても
     数MB程度）。グリフキャッシュはテーマ/フォントサイズ変更時に`invalidateGlyphCache()`で
     クリアされ、実描画された文字種の数だけに収まる。いずれも桁違いのメモリ消費源ではない。
  4. **`editor-buffer-architecture`スキルに記録済みの既知の制約**
     （`canvasCachedLines`＝文書全体を`String[]`で保持する設計、Stage④未着手）: 大容量ファイルで
     メモリを使う要因ではあるが、60箇所以上の呼び出し元が絶対行番号での同期アクセスを前提にしており、
     安全な解消には別途の設計・計画（絶対行番号ベースの遅延アクセスAPIへの置き換え）が必要と
     スキル側に明記されている。今回のセッションでは「ついで」に着手せず、既存の判断を踏襲して
     スコープ外とした。
  5. **JVMのデフォルト設定**（真因と判断）: `scripts/run.sh`/`run.bat`はこれまで`-Xmx`を
     一切指定しておらず、最大ヒープはJVM既定値（物理メモリの1/4）に委ねられていた。加えて
     G1GC は既定では`G1PeriodicGCInterval=0`（無効）のため、`javax.tools.JavaCompiler`による
     プロジェクト全体解析（`CompileAnalyzer.analyzeWithProject`、INSERT→NORMAL遷移や保存の
     たびに発火）やJDKクラス索引構築などで一時的にヒープ使用量が跳ね上がった後、アイドル状態が
     続いてもコミット済みメモリがOSへ返却されずそのまま高止まりする（JEP 346が有効化されて
     いない）。またG1のGC並列度（`ParallelGCThreads`/`ConcGCThreads`）とJITコンパイラスレッド数
     （`CICompilerCount`、既定で`CICompilerCountPerCPU=true`によりコア数に比例）はいずれも
     実行環境の論理コア数に応じてJVMが自動的に決定するため、コア数の多いマシンでは常駐スレッド数が
     数十に達する。これはリークではなく、単一ユーザー向けデスクトップアプリに対してサーバ向けの
     既定値がそのまま使われていることによる「無駄な太さ」であり、`-Xmx`未指定と合わせて
     RES高止まり・スレッド数過多の双方を説明する。
- **判断**: アプリケーションロジック（キャッシュ・スレッド生成・バッファ構造）には手を入れず、
  起動スクリプトのJVMフラグのみを変更する。理由: (a) 実際の原因はJVMの既定チューニングであり
  アプリ側に既知のリーク・不具合は見つからなかった、(b) `canvasCachedLines`等の設計変更は
  それ自体が大きな改修でありスキル側で意図的にスコープ外とされているため「ついでに」着手しない、
  (c) JVMフラグの変更はアプリの挙動（機能・出力）を一切変えず、ロールバックも`JAVA_OPTS`を
  未設定に戻すだけで容易。
- **実装**: `scripts/run.sh`/`scripts/run.bat`で、既に`JAVA_OPTS`が設定されていなければ
  以下を既定値として設定してから`java`を起動する（設定済みなら上書きしない）。
  - `-Xmx1536m`: 想定ファイル規模（数百行〜数十万行）とプロジェクト全体コンパイル解析の
    一時的な高負荷を吸収しつつ、無制限のヒープ拡張に上限を設ける。既存のmmap対応
    （`editor-buffer-architecture`スキル）によりファイルオープン自体はヒープを圧迫しないため、
    数十万行規模のテキスト編集・Undo履歴（上限1000件）・複数ペインを同時に開いても
    十分な余裕がある値として選定した。
  - `-XX:ParallelGCThreads=4 -XX:ConcGCThreads=1`: GCワーカースレッド数をコア数に依存させず
    固定する。単一ユーザーのGUIアプリでGCが動く時間は短く、並列度を上げても体感速度への
    寄与は小さい一方、コア数の多い環境での常駐スレッド数増加には直結する。
  - `-XX:CICompilerCount=2`: JITコンパイラ（C1/C2）スレッド数を固定する。ティアードコンパイル
    自体は無効化しない（`-XX:TieredStopAtLevel`等は変更していない）ため、JITによる実行時性能
    最適化は従来どおり効く。
  - `-XX:G1PeriodicGCInterval=60000`: アイドルが60秒続くとGCサイクルを走らせ、G1が
    （JDK 12以降のJEP 346により）未使用のコミット済みヒープ領域をOSへ返却できるようにする。
    `G1PeriodicGCSystemLoadThreshold`は既定値0（無効）のままのため、この定期GCが
    システム負荷を理由にスキップされることはない。
- **検証**: ソースコードは一切変更していないため、`./scripts/test.sh`のテスト結果は
  変更前と同じ既知のベースライン（`ModalEditorTest` 1件・`ScrollTest` 2件・`FilerTest` 1件、
  計3クラスFAILのみ・394テスト中の残り全PASS）のまま。加えてサンドボックス環境（4コア）で
  素のJVMプロセスとフラグ指定後のJVMプロセスを起動し比較したところ、`-Xmx`指定によりVSZ
  （予約仮想メモリ）が約6.9GiB→約4.3GiBへ縮小することを確認した（コア数が少ないため
  `ParallelGCThreads`等の既定値と指定値が一致し、スレッド数の差は本環境では確認できなかったが、
  高コア数環境ほど本フラグによるスレッド数削減効果は大きくなる設計であることをJVMの
  エルゴノミクス仕様から確認済み）。
- **代替案として検討したが不採用にしたもの**:
  - `-XX:+UseSerialGC`への切り替え: スレッド数はさらに減らせるが、Serial GCはStop-the-World時間が
    長くなりやすく、`javax.tools.JavaCompiler`によるプロジェクト全体解析のような一時的な
    大量アロケーションが発生する既存機能でUIフリーズが体感されるリスクがあるため見送った。
  - `CompileAnalyzer.analyzeWithProject`（INSERT→NORMAL遷移・保存のたびにプロジェクト全体を
    再walk・再コンパイルする実装）自体の見直し（差分解析・結果キャッシュ化等）: メモリ・CPUの
    両面で改善余地はあるが、`javac-compile-integration`/`auto-import-handler`スキルの守備範囲の
    大きな改修であり、複数ファイル間シンボル解決の正しさに関わるためこのセッション単体では
    着手しない。将来必要になった場合は対応スキルのSKILL.mdに設計判断ログとして残すこと。
