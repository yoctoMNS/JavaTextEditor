# 設計判断ログ（機能追加・不具合修正の詳細な経緯）

このドキュメントは、`CLAUDE.md`に時系列でベタ書きされていた「個々の機能追加・不具合修正ごとの詳細な設計判断ログ」を分離したものです（2026-07-28、プロジェクト構成整理の一環）。

- **`CLAUDE.md`との役割分担**: `CLAUDE.md`は「今のプロジェクトの決定済み設計・技術制約・作業方針」を簡潔にまとめた常時参照ファイルとして維持し、個々の判断に至った経緯・議論・却下した代替案・実機検証の詳細はすべてこちらに集約しています。`CLAUDE.md`はセッションのたびに全文がコンテキストへ読み込まれるため、頻繁に参照する必要のない過去の経緯まで含めると際限なく肥大化してしまう問題がありました（分離前は1,774行・約308KBに達していた）。
- **参照タイミング**: `.claude/skills/`配下の関連SKILL.mdを確認してもなお「なぜこの実装になっているか」「過去に何を試して却下したか」が知りたい場合に、このファイルの該当節（見出しはキーワードで検索可能）を参照してください。実装前に必ず読むべき第一の参照先は引き続き`.claude/skills/`のSKILL.mdと`CLAUDE.md`本体です。
- **記録の追加**: 新しい設計判断を行った場合の記録先は、対象機能に`.claude/skills/`配下の対応するSKILL.mdがあればそちらを優先し（プロジェクトの既定方針）、無ければこのファイルの末尾に追記してください。`CLAUDE.md`本体を書き換えるのは、決定済み設計事項の要約表・技術制約・ロードマップ等の「常時参照すべき現在の状態」が変わった場合のみとします。
- **並び順**: 元の`CLAUDE.md`にあった順序（おおむね時系列）をそのまま維持しています。
...
- **`:cd` の対象ディレクトリが存在しない場合、即座にエラーにせず y/n で新規作成確認するようにした**。従来は `WorkingDirectoryManager.setWorkingDirectory()` の `Files.exists()` チェックに落ちて `"E: ディレクトリが存在しません: ..."` を表示するだけだったが、「存在しないディレクトリを指定したら作成するか尋ね、y なら新規作成、n なら何もしない」という要望に基づき変更した。新設の `Mode.CD_CONFIRM_CREATE`（`KeymapRegistry` を経由せず `processCdConfirmCreateKey()` で直接キーを処理する、FILER/CLASSPATH_INPUT 等と同じ疑似モードパターン）を `ModalEditor.changeDirectory()` の入口に追加した。`Files.exists(target)` が false の場合は `changeWdCallback` を呼ぶ前に `cdConfirmTarget` へ対象パスを保存し `mode = Mode.CD_CONFIRM_CREATE` にしてプロンプトを表示するだけで return する（`processCommandKey` の Enter ハンドラは `if (mode == Mode.COMMAND) mode = modeAfterCommand();` という既存の判定のため、`FILER`/`BINARY` 復帰と同様この時点で `mode` が `COMMAND` から変わっていれば上書きされない）。`y`/`Y` で `Files.createDirectories(target)` の後に既存の「`changeWdCallback` 呼び出し→成功なら FILER へ」という処理（`applyChangeDirectory()` として共通化）を実行し、`n`/`N`/Esc は何もせず NORMAL モードへ戻る。既存の「ディレクトリが存在する場合の `:cd`」の経路（`applyChangeDirectory()`）自体は変更していない。
...
> **2026-07-29 追記**: 本節と後続の「Ctrl+Space 補完を WordIndex に一本化」節が述べる候補の集め方は、
> **`.java` バッファでは `.claude/skills/intellij-style-completion/SKILL.md` の実装に置き換わっている**
> （IntelliJ IDEA 方式のメンバー補完・並べ替え・確定処理）。
> C言語ファイル・ファイルパス未設定のバッファでは本節の記述が引き続き有効。
> **`Alt+/`（Vim の `i_CTRL-N` 相当）はファイル種別によらず本節のまま**であり、
> カーソルからの近接順という並び順も変更していない。
...
最小クラス）へ縮小する計画（`docs/archive/MAIN_DECOMPOSITION_PLAN.md`）を策定・全段階実行した。
判定基準は一貫して「`Main` に残ってよいのは部品を組み立てる記述だけ。`if`/`for` を含む処理は
1行も置かない」。段階6・7はそれぞれ専用のサブ段階分割計画書（`docs/archive/STAGE6_OPTION_C_PLAN.md`・
`docs/archive/STAGE7_PLAN.md`）を作り「1サブ段階=1コミット」で慎重に進めた。
...
`EditorApplication`）が生まれた。段階ごとの詳細・気づきは `docs/archive/MAIN_DECOMPOSITION_PLAN.md` §9
...
- **`EditorHost`（`PaneManager` が実装する、`ModalEditor` → ペイン管理への窓口インタフェース）
  ...
  **例外が1つだけある**: `setHost()` は `setCloseBlockedCallback` を意図的に配線しない。
  ...
  Xvfb+Robot手動検証で実際にこの回帰を発見・修正した（詳細は `docs/archive/STAGE6_OPTION_C_PLAN.md`
...
`docs/archive/MAIN_DECOMPOSITION_PLAN.md` §8 に記載のとおり本計画のスコープ外のまま。
...
## mmap経路のCRLF/BOM対応（大容量ファイル対応 Phase 3 続報・2026-08-02）

- **背景**: 上記「大容量ファイル対応（mmap化）」のレビューで、「mmap経路（8MiB以上）はCRLF
  正規化・BOM除去を行わない」という初版のスコープ境界が、性能とは別軸のユーザー可視バグ
  （同じ内容のファイルでもサイズだけで改行・BOMの見え方が変わる）に該当するという指摘を受け、
  Stage④（ビューポート限定描画）より優先して対応した。
- **対応方針の検討**: 「デコード後の文字列に対して`\r\n`除去を行う」という素朴な案は、
  `getTextInRange`が返す文字列の長さが要求した文字数（`toOffset - fromOffset`）と一致すると
  いう既存の暗黙の前提を壊す（除去した分だけ短くなる）ため、そのままでは採用できないと判断した。
  最終的に採用した方式: `LazyLineIndex`の文字カウント自体に「直後が`\n`である`\r`は0幅」という
  ルールを追加し、文字オフセットの境界が原理的に`\r`と`\n`の間で割れないようにしたうえで、
  デコード後の文字列に単純な`"\r\n"→"\n"`置換をかける。境界が割れない保証があるため、この
  置換は前提を壊さない（要求文字数と結果の文字列長が必ず一致する）。BOMは`LazyLineIndex`の
  走査開始バイトオフセットを3バイトずらすだけで対応（全文コピー不要）。
- **判断が正しいことの確認**: 既存の小規模ファイル経路（`Files.readAllBytes`+
  `new String(...).replace("\r\n","\n")`）が孤立した`\r`（`\n`を伴わない旧Mac形式）を
  変更しない仕様であることを確認し、mmap経路も同じ仕様（`\r\n`ペアのみ対象、孤立`\r`は保持）に
  揃えた。
- **テスト**: `MappedCrlfBomTest`（新規、12/12）でCRLF単体・孤立`\r`・BOM単体・BOM+CRLF複合・
...
## 起動時JVMメモリ/GCチューニングの過剰修正を撤回し、アイドル専用の定期GCのみへ縮小（2026-08-04）

- **経緯**: 前回、システムモニタ/htopでのアイドル時RES高止まり（9〜10GiB前後）とスレッド数の
  多さ（htopのLWP行が70件以上）を受け、`scripts/run.sh`/`run.bat`へ`-Xmx1536m`
  `-XX:ParallelGCThreads=4` `-XX:ConcGCThreads=1` `-XX:CICompilerCount=2`
  `-XX:G1PeriodicGCInterval=60000`を追加した（コミット`3d83dec`）。この修正の直後、
  「操作時にカクつく（応答が遅延する）」という体感パフォーマンスの著しい悪化が報告された。
- **原因の切り分け**: `3d83dec`はアプリケーションコード（キャッシュ・オブジェクト生成頻度・
  バッファ構造）を一切変更しておらず、`scripts/run.sh`/`run.bat`のJVM起動フラグのみが差分
  だった（`git show --stat 3d83dec`で確認）。したがって「キャッシュ破棄」「オブジェクト生成
  頻度」自体は変化しておらず、影響しうるのはGC関連フラグのみに絞られる。
  - **`-XX:ParallelGCThreads=4 -XX:ConcGCThreads=1`が主因と判断**: これらはJVMの既定値
    （実行環境の論理コア数に応じてエルゴノミクスが自動決定する値）を、検証に使ったサンドボックス
    環境（4コア）に合わせた固定値へ上書きしてしまっていた。ユーザーの実機は前回のhtop
    スクリーンショット（多数のGC/JITスレッドが常駐）から推測して4コアよりかなり多いコア数を
    持つと考えられ、そのような環境で並列GCスレッド数を4・並行GCスレッド数を1に固定すると、
    実際にGCが発生した際のポーズ時間（Stop-the-World時間）がハードウェアの並列度なりに
    短縮されず、コア数の少ない環境と同じだけの時間がかかるようになる。これが「操作時にカクつく」
    の直接的な原因と推測される。
  - **`-Xmx1536m`も一因と推測**: ヒープ上限を1536MBに固定したことで、実際のワーキングセット
    （大きめのファイル・プロジェクト全体のjavac解析・複数ペイン等）がこれに近い、または
    超える場合にGCの発生頻度自体が従来（ヒープ上限が物理メモリの1/4という大きな既定値）より
    増えた可能性がある。上記のGCスレッド数固定と組み合わさることで、頻度・ポーズ時間の両面で
    悪化したと考えられる。
  - **`-XX:CICompilerCount=2`も一因と推測**: JITコンパイラ（C1/C2）スレッド数を固定したことで、
    ホットパス（キー入力処理・描画ループ等）の最適化コンパイルが従来より遅く進み、体感速度の
    低下に寄与した可能性がある。
  - **`-XX:G1PeriodicGCInterval=60000`は主因ではないと判断**: この定期GCはアロケーションが
    実質止まっている（＝ユーザーが操作していない）期間にのみ発火する仕組み（JEP 346）であり、
    連続したタイピング・スクロール等の「操作中」には発火しない。「操作時にカクつく」という
    報告内容とは発生タイミングが一致しないため、主因からは除外した。
- **対応**: `3d83dec`を`git revert`で機械的に取り消したうえで（他のコミットとの競合は無く、
  `docs/decision-log.md`のみ末尾追記同士の自動マージで解決）、単純に全フラグを削除するのではなく、
  上記の原因分析で「対話操作中の応答性に影響しない」と判断した`-XX:G1PeriodicGCInterval=60000`
  のみを最小構成として再導入した。`-Xmx`・`ParallelGCThreads`・`ConcGCThreads`・
  `CICompilerCount`はいずれも既定のJVMエルゴノミクス（実行環境のコア数・物理メモリに応じた
  自動決定）に委ねる形へ戻し、二度と固定値で上書きしない。
- **Eclipse等のIDEが採用する省メモリ手法の適用検討（本セッションで依頼された再設計方針）**:
  ユーザーからは「Eclipse方式」として遅延読み込み・仮想描画・インクリメンタル解析・
  `SoftReference`キャッシュ・バックグラウンド分離・差分Undo/Redoの6パターンの適用を依頼されたが、
  精査の結果、いずれも「今回の性能劣化の原因（JVM起動フラグ）とは無関係」であり、かつ
  以下の理由から本セッションでの新規実装は見送った（詳細はユーザーへの報告メッセージ本文、
  および各項目末尾に記載の参照先スキルを参照）。
  1. **遅延読み込み**: `editor-buffer-architecture`スキルのPhase 3（mmap化）で実装済み。対応不要。
  2. **バックグラウンドスレッド分離**: `LiveDiagnostics`・`WordIndex`・`JdkClassIndex`等が
     既に`Thread.ofVirtual()`でUIスレッドをブロックせず実行済み。対応不要。
  3. **仮想描画（ビューポート限定描画）**: `editor-buffer-architecture`スキルに
     「Stage④は未着手」として明記済み。`canvasCachedLines`（文書全体の`String[]`）を
     60箇所以上が絶対行番号での同期アクセス前提で参照しており、安全な移行には専用の
     設計・段階的移行計画が必要とスキル側に既に明記されている。今回のカクつき修正の
     スコープを超える大改修であり、「既存機能への影響は一切禁止」という制約と両立させるには
     このセッション単体では検証しきれないため見送った。着手する場合は別途専用セッションで
     `editor-buffer-architecture`スキルの計画に従うこと。
  4. **インクリメンタル解析**: 構文ハイライトの一部（ブロックコメント判定）は既に
     インクリメンタル化済み（コミット`4afdf7d`）。一方`CompileAnalyzer.analyzeWithProject`
     （保存・INSERT離脱のたびにプロジェクト全体を再walk・再コンパイル）は非インクリメンタルの
     ままで、これは前回のメモリ調査時点で「複数ファイル間シンボル解決の正しさに関わるため
     このセッション単体では着手しない」と既に判断・記録済み（本ファイル前節）。今回もその
     判断を踏襲し、着手していない。
  5. **`SoftReference`キャッシュ**: `EditorCanvas`のグリフキャッシュ（`glyphCacheFg/Bg`・
     `uiGlyphCache`・`nonAsciiGlyphCache`）はテーマ/フォントサイズ変更時に`invalidateGlyphCache()`
     で明示的にクリアされる設計で、保持数も実描画された文字種数に収まる小さなキャッシュのため、
     `SoftReference`化しても得られる効果は小さいと判断し見送った。
  6. **差分ベースのUndo/Redo**: `CLAUDE.md`の「決定済みの設計事項」表に
     「ピースリストのスナップショット方式」と明記され、`editor-buffer-architecture`スキルに
     専用の参照ドキュメント（`piece-table-delete-and-undo.md`）まで存在する、**既に確定した
     アーキテクチャ上の決定**である。これを差分（diff）ベースへ置き換えることは、Undo/Redo・
     VISUAL BLOCK編集・`:s`置換・マクロ再生等、多数の既存機能（テスト1000件超）に影響する
     大規模な再設計であり、CLAUDE.mdの「既存のSkillの内容と矛盾する実装をしようとしている
     場合は、実装を進める前にユーザーに確認すること」という方針に従い、ユーザーへの確認なしに
     本セッションで着手することはしなかった。
- **検証**: ソースコード（`src/`配下）は一切変更していないため、`./scripts/test.sh`は
  既知のベースライン（`ModalEditorTest` 1件・`ScrollTest` 2件・`FilerTest` 1件、計3クラス
  FAILのみ）から変化なし。

## Javaファイル編集時にメモリ使用量が青天井に増える問題の修正（2026-08-07）

- **報告された症状**: テキストエディタでJavaファイルを開いて起動しっぱなしにしていると、
  2分程度でメモリ使用量が4,000MB以上になり、以後も下がらない。Emacsで同じことをしても28MB。
- **調査方法**: Xvfb上で実機と同じ起動経路（`Main`）から起動し、
  ①`jcmd GC.heap_info` / `/proc/<pid>/status` によるヒープ・RSSの時系列測定、
  ②JFR（`settings=profile`）の `jdk.ObjectAllocationSample` を「起動処理が終わった後」だけに
  絞って割り当て元別に集計、③`com.sun.management.ThreadMXBean#getThreadAllocatedBytes` による
  個別APIの割り当て量実測、の3点で切り分けた。
- **4,000MBという数字の正体**: `-XX:MaxHeapSize` の既定値は物理メモリの1/4で、
  16GB機ではちょうど約4,020MB。つまり「上限まで膨らんで戻らない」状態であり、
  参照が残り続ける古典的なリークではなく **割り当て速度が異常に高いこと** が原因だった。
  `scripts/run.sh` の `-XX:G1PeriodicGCInterval=60000`（前回の対策）は
  「その間隔中にGCが1回も起きなかった＝完全にアイドル」なときにしか発火しないため、
  常時ゴミが出ている状態では一度も効かない。

### 原因1（支配的）: 編集のたびにプロジェクト全体を再コンパイルしていた

`CompileAnalyzer.analyzeSourceWithProject()` が、解析要求のたびに作業ディレクトリ配下の
全 `.java` を読み込んで丸ごと `javac` に渡していた。このプロジェクト自身（約27,000行）で
**1回あたり約371〜418MB・0.8〜3.2秒**。解析はINSERT離脱・保存・バッファ変更（400msデバウンス）で
駆動されるため編集中は数秒に1回発生し、しかも `LiveDiagnostics` が要求ごとに
`Thread.ofVirtual().start()` していたので数百MB級の作業メモリが同時に何本も生存しえた。
Javaバッファでしか通らない経路のため、「Javaファイルを開いたとき」という報告とも一致する。

修正は ①javacの `-sourcepath` を使い編集中のバッファ1件だけをコンパイル対象にする方式へ変更
（新規 `dev.javatexteditor.analysis.JavaSourceRoots` がソースルートを算出）、
②解析を単一スレッドに直列化し、追い越された要求はjavacを動かす前に破棄、
③同一内容のソースに対する再解析は診断をキャッシュから返す、の3点。
実測で **12〜65MB / 0.02〜0.5秒** になり、診断結果（クロスパッケージ解決・
import欠落の `cannot find symbol`）は従来と同一であることを確認済み。
詳細・実測値・テストは `.claude/skills/javac-compile-integration/SKILL.md`
「メモリ肥大化の修正: 編集のたびにプロジェクト全体を再コンパイルしていた問題（2026-08-07）」を参照。

### 原因2: アイドル時も描画・監視まわりが割り当てを続けていた

操作していなくても、30fpsのステータスライン再描画と2秒周期のシステム統計取得が
ゴミを出し続けていた（歩行スプライトのピクセルごとの `new Color`、
グリフキャッシュの `computeIfAbsent` に渡すラムダ、Linuxでは無意味な
毎秒1000回の `Thread.sleep(1)` スレッド、存在しない `nvidia-smi` の永久リトライ、
`/proc/meminfo` の `readAllLines` と行ごとの正規表現）。
いずれも **描画結果を1ピクセルも変えない範囲で** 取り除いた。詳細と、
「軽そうに見えるが実測でピクセルが変わるため採らなかった案」は
`.claude/skills/gui-rendering-pipeline/SKILL.md`
「アイドル時に描画経路がゴミを出し続けていた問題の修正（2026-08-07）」を参照。

### JVMオプションは変更していない

`scripts/run.sh` / `run.bat` の `-XX:G1PeriodicGCInterval=60000` はそのままにした。
2026-08-04に「起動時JVMメモリ/GCチューニングの過剰修正を撤回」した経緯（本ファイル前節）のとおり、
`-Xmx` 等でヒープ上限を絞るのは大容量ファイル編集という本エディタの想定用途と衝突するし、
そもそも今回の原因は割り当て速度であってGCの設定ではない。割り当て速度が下がった結果、
実測ではアイドル時に周期GCが発火してヒープが縮む（260MB→75MB・RSS 287MB→141MB）ことも確認した。
