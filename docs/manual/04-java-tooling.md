...
1. `receiver.member` 形式でカーソルが `member` 側にある場合、`receiver` をプロジェクト内のクラスとして解決し、そのメンバー宣言を探す。プロジェクト内で見つからなければ `receiver` の型（ローカル変数・引数・フィールドの宣言から推定）をJDKクラスとして解決を試みる
2. プロジェクト全体からカーソル位置の識別子と一致するフィールド・定数・メソッド・クラス宣言を検索し、見つかればそのファイル・行へジャンプ
3. 上記までのいずれにも該当しない場合、カーソル位置の識別子自体がローカル変数・引数の名前である可能性を試す（`receiver.member` の `receiver` 側にカーソルがある場合を含む）。宣言箇所（`Type varName` の形が現れる直近の行）が見つかれば、同一ファイル内でその行へジャンプする
4. JDKソース疑似バッファ内でnativeメソッドの実装（C/C++）を参照中の場合は `lib/openjdk-native/` から該当シンボルを検索
5. JDKソース疑似バッファ内でJavaのFQN表示中の場合、native実装のJNIマングル名トレースを試みる
6. `ClassName.methodName` 形式でカーソルがある場合、nativeメソッドかどうかを検出しJNIマングル名をトレース
7. 上記のいずれにも該当しない場合、JDKクラス名として検索する（下記「JDK API参照」を参照）
...
| キー/コマンド | 動作 |
|---|---|
| `Space+i+o` | 未使用importを一括削除（Eclipse互換。NORMAL/INSERT両モードで有効） |
...
> **言語による自動切り替え**: `F10`/`F11`/`F12` は「現在開いているファイルの拡張子」で動作が切り替わります。`.java` を開いていれば以下のJava（`javac` 相当）の動作、`.c`/`.h` を開いていればC（`gcc` 相当）の動作になります。C版の詳細は [C言語開発支援](11-c-tooling.md) を参照してください。
