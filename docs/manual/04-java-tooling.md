...
- **`F10`（コンパイル）**: `javax.tools.JavaCompiler` で作業ディレクトリ配下の全 `.java` を走査してコンパイルし、`.class` を `<projectRoot>/bin` に出力します（外部 `javac` プロセスは起動しません）。`.git`/`build`/`target`/`.gradle`/`node_modules`/`.idea`/`.vscode`・出力先自身の `bin/` はスキャン対象から除外されます。結果は `*compile*` 疑似バッファに表示されます
- **`F11`（実行）**: `public static void main(String[])` を持つクラスを作業ディレクトリ配下から探して実行対象を決定します。1件ならそのまま実行、複数見つかった場合はtelescope風の3ペインオーバーレイから選択します。`F10` を一度も実行しておらず `bin/` が空の場合はエラーを表示するだけで実行しません。実行は別プロセス（`java -cp <projectRoot>/bin <FQCN>`）として起動され、標準出力・標準エラーはマージして捕捉し、プロセス終了後にまとめて `*run*` 疑似バッファに表示します（リアルタイムのストリーミング表示ではありません）。そのため `Scanner` 等で標準入力を要求する対話的なプログラムは正しく動作しません
