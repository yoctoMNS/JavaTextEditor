...
public class EditorCanvas extends JPanel implements InputMethodListener {
     ...
     * 矢印(→←↑↓)・罫線(─│┌┐└┘├┤┬┴┼)・幾何学模様(■□▲△▼▽◆◇○●)等、
     * 日本語の文章・箇条書き・ASCIIアート的な罫線表現で一般的に全角幅で
     * 描画される記号ブロックを網羅する。
     * 厳密なUnicode East Asian Width判定（Ambiguous/Wideの完全な区別）は行わず、
     * 日本語環境で慣習的に全角表示される範囲を実用上十分な精度でカバーする
     * （Braille Patterns(0x2800-0x28FF)は視覚障害者向けの点字ブロックで、
     * どのフォントでも常に半角(Neutral)扱いのため、下記2100-2BFFの中でも
     * ここだけは意図的に除外している）。
     */
    private static final int[][] WIDE_RANGES = {
        // 字は非有／その他の記号ブロック（Letterlike Symbols〜Supplemental Arrows-A）:
        // 数字に準じる記号・ローマ数字(Ⅰ Ⅱ Ⅲ)・矢印(→←↑↓)・数学記号・
        // 囲み英数字(①②③)・罫線(─│┌┐)・ブロック要素(█▓▒░)・幾何学模様(■□●)・
        // その他の記号(☆★)・装飾記号を丸ごと含む。個別に列挙すると際限なく漏れが
        // 発生するため(実際に矢印・罫線が漏れて重なる不具合が起きた)、意味的に
        // 連続するこの範囲はまとめて全角とする。
        {0x2100, 0x27FF},
        {0x2900, 0x2BFF}, // 補助矢印B・その他の数学記号・その他の記号と矢印(Braille直後〜)
