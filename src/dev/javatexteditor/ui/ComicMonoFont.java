...
/**
 * Comic Mono Regular (TTF, MIT License) を使って半角ASCII (0x20-0x7E) を
 * 描画するフォント。:font 3 で選択できる（既定は MiscFixedBold9x15、:font 0）。
 * 配布元: https://dtinth.github.io/comic-mono-font/ （実体は GitHub リポジトリ
 * dtinth/comic-mono-font）。実装は {@link IbmPlexMonoFont} と同一の設計（事前生成した
 * 固定サイズのビットマップ集合を持たず、任意の cellW×cellH に対してベクターアウトラインを
 * 非等方向にスケールしてラスタライズする）をそのまま踏襲している。
 *
 * TTF実体は lib/fonts/ComicMono.ttf に配置される（scripts/setup.sh/setup.bat が
 ...
 * 既定高さ15pxに当てはめて幅を逆算）で求めた値。Comic Mono Regular の実測は
 * ascent=80・descent=32・advance('M')=55・比率≒0.4911 で、round(15*0.4911)=7 となった。
 */
public final class ComicMonoFont implements MonoFont {
    ...
    public static final ComicMonoFont INSTANCE = new ComicMonoFont();
    ...
    /**
     * lib/fonts/ComicMono.ttf を実際に読み込めたかどうかを返す。
     * false の場合は Font.MONOSPACED へフォールバック済み（起動・描画自体は継続する）。
     * :font 3 実行時にステータスバーへ setup.sh/setup.bat の実行を促す表示を出すために使う
    ...
    private static Path findTtf() throws IOException {
        var found = CodeSourceLocator.findUpward(
            ComicMonoFont.class, "lib/fonts/ComicMono.ttf", 4, Files::exists);
        if (found.isPresent()) return found.get();
        Path fromCwd = Path.of("lib", "fonts", "ComicMono.ttf");
